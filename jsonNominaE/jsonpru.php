...
$anno=$reAnno[0];
        ...
        $sqlTercero="SELECT tercero  FROM gn_empleado
        WHERE id_unico=$empleado";
        $resultTer = $mysqli->query($sqlTercero);
        $rowTer = $resultTer->fetch_assoc();
        $tercero = $rowTer['tercero'];
        ...
        echo $sqlHENDF="SELECT c.equivalente_NE as tipo,SUM(n.valor) AS valor,SUM(n1.valor) as cantidad
                        FROM gn_novedad n 
                        LEFT JOIN gn_concepto c ON c.id_unico=n.concepto
                        LEFT JOIN gn_periodo p ON p.id_unico=n.periodo
                        LEFT JOIN gn_concepto c1 ON  c1.id_unico=c.conceptorel
        ...
        while ($rowHENDF=mysqli_fetch_row($horasHENDF)) {
           ...
           echo $horasExtrasNocturnaDomFes=(int)$horasExtrasNocturnaDomFes1;
...
while ( $rowDeduccionSub=mysqli_fetch_row($deduccionSub)) {
    ...
    while ($rowDeuda=mysqli_fetch_row($deuda)){
        ...
        /* "horasExtrasRecargos"    =>[ $horasExtrasDiurnasJson
                                            ],
                "vacaciones"             => [$vacacionesJson],
            
                "incapacidades"         => [$incapacidadesComunJson,
                                            $incapacidadesProfesionalJson,
                                            $incapacidadesLaboralJson], 

            "licencias"              =>[ $licenciaMPJson,
                                            $licenciaReJson,
                                            $licenciaNReJson
                                        ], 
            "otrosDevengados"        =>[$viaticosManuJson,
                                        $viaticosManuNSJson,
                                        $bonificacionSJson,
                                        $bonificacionNSJson,
                                        $auxiliosNSJson,
            ...
            "deducciones" => [
                ...
                "deduccionesValor"   =>[ $sancionPublicaJson,
                                        $sancionPrivadaJson,
                                        $pagotercerDedeuccionesJson,
                                        $anticipoJson,
                                        $otraDeduccionJson,
        ...
        /* $nomina["horasExtrasRecargos"]=[$horasExtrasDiurnasJson
        ]; */
        ...
        if (count($arrayTempoOtrosDe)>0) {
            if (count($primasJson)>0) {
                if(count($arrayTempoInca)>0 || count($arrayTempoLicen)>0){
