...
@$id = $_GET['idE'];
$emp = "SELECT e.id_unico, e.tercero, IF(CONCAT_WS(' ',
         t.nombreuno,
         t.nombredos,
         t.apellidouno,
         t.apellidodos) 
         ...
         t.nombredos,
         t.apellidouno,
         t.apellidodos)) AS NOMBRE , t.tipoidentificacion, ti.id_unico, CONCAT(ti.nombre,' ',t.numeroidentificacion)
FROM gn_empleado e
...
<script>
   ...
   <title>Liquidación Nómina</title>
    <link rel="stylesheet" href="css/select2.css">
        <link rel="stylesheet" href="css/select2-bootstrap.min.css"/>
    </head>
    <body>
              ...
              <div class="row content">
                  ...
                  <div class="col-sm-8 text-left" style="margin-top: 0px">
                      ...
                      <div class="client-form contenedorForma" style="margin-top: -7px;font-size: 13px">
                          <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="json/liquidarNominaJson.php">
                      ...
                      <div class="form-group form-inline" style="margin-top:-25px">
                       ...
                       if(empty($idT))
                        ...
                        ?>
                          ...
                          <!--------------------------------------------------------------------- -->
                            <?php
                                $per = "SELECT  p.id_unico, CONCAT(p.codigointerno,' - ',tpn.nombre) FROM gn_periodo p LEFT JOIN gn_tipo_proceso_nomina tpn ON p.tipoprocesonomina = tpn.id_unico WHERE p.id_unico!=1 AND p.liquidado !=1 AND tipoprocesonomina = 1 AND p.parametrizacionanno = $panno  ";

                                $periodo = $mysqli->query($per);
                            ?>

                            
                            <label for="sltPeriodo" class="col-sm-2 control-label">
