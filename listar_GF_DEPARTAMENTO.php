...
switch ($_SESSION['conexion']) {
    case '1':
      $row = $con->Listar("SELECT d.id_unico, d.nombre,p.nombre,d.rss FROM gf_departamento d INNER JOIN gf_pais p ON p.id_unico=d.pais");
    break;
    case '2':
      $row = $con->Listar("SELECT d.id_unico, d.nombre,p.nombre,d.rss FROM gf_departamento d INNER JOIN gf_pais p ON p.id_unico=d.pais");
    break;
            ...
            <?php
               ...
               <tr>
                ...
                <td><!-- Hipervínculos para eliminación y modificación de los registros. -->
                  <a class href="#" onclick="javascript:eliminarDepartamento(<?php echo $row[$i][0];?>);"><i title="Eliminar" class="glyphicon glyphicon-trash"></i></a>
                  <a href="modificar_GF_Departamento.php?id_departamento=<?php echo ($row[$i][0]);?>"><i title="Modificar" class="glyphicon glyphicon-edit" ></i></a>
                </td>
      ...
      function eliminarDepartamento(id)
      {
        $("#myModal").modal('show');
                        $("#ver").click(function(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificar_GF_DepartamentoJson.php?action==1",
                                data: form_data,
