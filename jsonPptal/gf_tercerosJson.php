...
switch ($action) {
    ...
    case 10:
        ...
        $sql_dato = array(
            array(":tercero", $_REQUEST['tercero']),
            array(":responsabilidad", $_REQUEST['responsabilidad']),
        );
    ...
    case 14:
        ...
        $id         =  $_REQUEST['id'];
        $tipoIden   =  $_REQUEST['tipoIdent'];
        $noIdent    =  $_REQUEST['noIdent'];
        $digitVerif = $_REQUEST['digitVerif'];
        $razoSoci   =  $_REQUEST['razoSoci'];
        ...
        $perfilid = $_REQUEST['perfil'];
        $primerN    = $_REQUEST['primerN'];
        $primerA    = $_REQUEST['primerA'];
        $segundoN   = $_REQUEST['segundoN'];
        $segundoA   =  $_REQUEST['segundoA'];
        $codigoE     =  $_REQUEST['nominaE'];
        $tipoEnt    =  $_REQUEST['tipoEntidad'];
        ...
        Sucursal = :sucursal, RepresentanteLegal = :repreLegal, 
         TipoRegimen = :tipoReg, ciudadidentificacion = :ciudad,
        Contacto = :contacto, TipoEmpresa =:tipoEmp, 
        Zona = :zona, email = :correo, NombreUno=:primerN, 
        NombreDos=:segundoN, ApellidoUno=:primerA, ApellidoDos=:segundoA, equivalente_NE = :codigoE, TipoEntidad =  :tipoEnt, nombre_comercial = :nombreC 
        WHERE Id_Unico = :id_unico";

        $sql_dato = array(
            array(":id_unico", $id),
            array(":tipoIden", $tipoIden),
            array(":noIdent", $noIdent),
            array(":digitVerif",  $digitVerif),
            array(":razoSoci", $razoSoci),
            array(":ciudad", $ciudad),
            array(":tipoReg", $tipoReg),
            array(":tipoEmp", $tipoEmp),
            array(":sucursal", $sucursal),
            array(":contacto", $contacto),
            array(":repreLegal", $repreLegal),
            array(":zona", $zona),
            array(":correo", $correo),
            array(":primerA", $primerA),
            array(":primerN", $primerN),
            array(":segundoN", $segundoN),
            array(":segundoA", $segundoA),
            array(":codigoE", $codigoE),
            array(":tipoEnt", $tipoEnt),
            array(":nombreC", $nombreC),
        );
        ...
        $perfil = $con->Listar("select perfil from gf_perfil_tercero  where perfil=$perfilid and tercero=$id");
        if (empty($perfil[0][0])) {



            $sqlconsperfil = "INSERT INTO gf_perfil_tercero (Perfil, Tercero) VALUES($perfilid, :id_unico)";
            $sql_datoperfil = array(
                array(":id_unico", $id),
                array(":perfil", $perfilid),
            );

            $obj_resperfil = $con->InAcEl($sqlconsperfil, $sql_datoperfil);
            if (empty($obj_resperfil)) {
                echo 1;
            } else {
                echo 2;
                echo ($id);
    ...
    case 15:
        ...
        $row  = $con->Listar("SELECT MAX(Id_Unico) AS Id_Unico FROM gf_tercero");
        $finalid = $row[0][0] + 1;

        $sql_cons = "INSERT INTO gf_tercero (id_unico, RazonSocial, NumeroIdentificacion, 
        DigitoVerficacion, Compania, TipoIdentificacion, Sucursal, RepresentanteLegal, 
        CiudadIdentificacion, TipoRegimen, Contacto, TipoEmpresa,  Zona, email,NombreUno, 
        NombreDos, ApellidoUno, ApellidoDos, TipoEntidad  ,equivalente_NE)
        VALUES(:finalid, :razoSoci, :noIdent, 
        :digitVerif, :compania, :tipoIden, :sucursal, :repreLegal, 
        :ciudad, :tipoReg, :contacto, :tipoEmp,:zona, :correo, :primerN, 
        :segundoN, :primerA, :segundoA, :tipoEnt,:codigoE )";
        ...
        $idaux = $row[0][0];
        $sql_consperfil = "INSERT INTO gf_perfil_tercero (Perfil, Tercero) VALUES(:perfil, :idaux)";
        ...
        $obj_respperfil = $con->InAcEl($sql_consperfil, $sql_datoperfil);
