...
$bdb             = buscardependencia('Bodega Almacen');
if($bdb==0){
    creardependencia('999','Bodega Almacen', 1, 0);
    $bdb = buscardependencia('Bodega Almacen');
}
...
if (!empty($_FILES['file']['tmp_name'])) {
    ...
    if ($objPHPExcel->setActiveSheetIndexByName('INVENTARIO GENERAL')) {
        ...
        //var_dump('Total Filas '.$total_filas);
        $ti             = 2;
        $tf             = 10;
            ...
            for ($a = $ti; $a < $tf; $a++) {
                ...
                if($total_filas<=0){
                    ...
                    if(empty($sede)){
                        ...
                        $bdb = buscardependenciaSigla($sede);
                        if($bdb==0){
                            creardependencia($sede, $dependen, 5, 0);
                        }
                        $id_sede = buscardependenciaSigla($sede);
                    }
                    #DEPENDENCIA
                    if(empty($dependen)){
                        ...
                        if($id_responsable==0){
                            ...
                            if($tme==0){
                                ...
                                if(empty($elemento)){
                                    ...
                                    #************ CRER MOVIMIENTOS *************#
                                    #ENTRADAS
                                    $entrada = buscarMovimiento($tme, $id_dependencia, $id_responsable);
                                    if($entrada ==0){
                                        $ms = crearMovimiento($tme, $id_dependencia, $id_responsable);
                                    } 
                                    $entrada = buscarMovimiento($tme, $id_dependencia, $id_responsable);

                                    #SALIDAS 
                                    $salida = buscarMovimiento($tms, $id_dependencia, $id_responsable);
                                    if($salida ==0){
                                        $ms = crearMovimiento($tms, $id_dependencia, $id_responsable);
                                    } 
                                    $salida = buscarMovimiento($tms, $id_dependencia, $id_responsable);
...
function buscardependenciaSigla($sigla){
    global $con;
    global $compania;
    
    $nombre = str_replace(' ', '', $sigla);
    $buscar = $con->Listar("SELECT * FROM gf_dependencia
        WHERE REPLACE(sigla, ' ', '')='$sigla'");
    if(!empty($buscar[0][0])){
        $id = $buscar[0][0];
    } else {
        $id = 0;
...
function buscarMovimiento($tipo, $dependencia, $responsable){
    global $con;
    global $compania;
    global $nanno;
...
function crearMovimiento($tipo, $dependencia, $responsable){
    global $con;
    global $compania;
    global $parametrizacion;
    global $fecha;
    ...
    $sql_dato = array(
        array(":numero",$numero),
        array(":fecha",$fecha),
        array(":descripcion",'Movimiento Inventario Inicial'),
...
function guardarElemento($codigoP, $nombre, $ti,$nombre_a){
    ...
    WHERE codi LIKE '$codigoP%'  AND compania = $compania AND codi !='$codigoP'" );
    if(empty($cd[0][0])){
