...
switch ($action) {
    ...
    case 1:
        ...
        if(empty($_REQUEST['ctaIva'])){
            $ctaIv = NULL;
        } else {
            $ctaIv = $_REQUEST['ctaIva'];
        }
        if(empty($_REQUEST['ctaImpo'])){
            $ctaIm = NULL;
        } else {
            $ctaIm = $_REQUEST['ctaImpo'];
        }       

        
        $sql_cons ="INSERT INTO `gf_configuracion_almacen` 
            ( `plan_inventario`, `tipo_movimiento`, `cuenta_debito`,`cuenta_credito`,`parametrizacion_anno`, `cuenta_iva`, `cuenta_impo` ) 
            VALUES (:plan_inventario, :tipo_movimiento, :cuenta_debito, :cuenta_credito, :parametrizacion_anno, :cuenta_iva, :cuenta_impo)";
	    $sql_dato = array(
            ...
            array(":cuenta_iva",$ctaIv),
            array(":cuenta_impo",$ctaIm),
	    );
    ...
    case 2:
        ...
        $obj_resp = $obj_con->InAcEl($sql_cons,$sql_dato);
        if (empty($obj_resp)) {
            $rowc1 = $obj_con->Listar("SELECT codi FROM gf_plan_inventario WHERE id_unico = $grupo");
            $rowp = $obj_con->Listar("SELECT id_unico, tienemovimiento FROM gf_plan_inventario WHERE compania = $compania AND codi like '".$rowc1[0][0]."%'");
            ...
            if(count($rowp)>0){
                for ($i = 0; $i < count($rowp); $i++) {
                    ...
                    $vlg = $obj_con->Listar("SELECT * FROM gf_configuracion_almacen 
                        WHERE plan_inventario=$plan_i 
                        AND parametrizacion_anno=$anno AND tipo_movimiento IS NULL");
                    if(count($vlg)>0){ } else {
                        $sql_cons ="INSERT INTO `gf_configuracion_almacen` 
                            ( `plan_inventario`, `cuenta_debito`,`cuenta_credito`,`cuenta_baja`,`parametrizacion_anno` ) 
                            VALUES (:plan_inventario, :cuenta_debito, :cuenta_credito, :cuenta_baja,:parametrizacion_anno)";
            ...
            if(count($rowp)>0){
                for ($i = 0; $i < count($rowp); $i++) {
                    ...
                    if(empty($tipom)){
                        $vlg = $obj_con->Listar("SELECT * FROM gf_configuracion_almacen 
                        WHERE plan_inventario=$plan_i 
                        AND parametrizacion_anno=$panno AND tipo_movimiento IS NULL ");
                    } else {
                        $vlg = $obj_con->Listar("SELECT * FROM gf_configuracion_almacen 
                        WHERE plan_inventario=$plan_i 
                        AND parametrizacion_anno=$panno AND tipo_movimiento=$tipom");
    ...
    case 4:
        ...
        $ctade = $_REQUEST['ctad'];
        $ctacr = $_REQUEST['ctac'];
        if(empty($_REQUEST['ctaIv'])){
            $ctaIv = NULL;
        } else {
            $ctaIv = $_REQUEST['ctaIv'];
        }
        if(empty($_REQUEST['ctaIm'])){
            $ctaIm = NULL;
        } else {
            $ctaIm = $_REQUEST['ctaIm'];
        }      
                ...
                for ($i = 0; $i < count($rowp); $i++) {
                    ...
                    if(count($vlg)>0){
                        ...
                        $sql_cons ="INSERT INTO `gf_configuracion_almacen` 
                            ...
                            VALUES (:plan_inventario, :tipo_movimiento, :cuenta_debito, :cuenta_credito, :parametrizacion_anno, :cuenta_iva, :cuenta_impo)";
                        $sql_dato = array(
                            array(":plan_inventario",$grupomod),
                            array(":tipo_movimiento",$tipom),
                            array(":cuenta_debito",$ctade),
                            array(":cuenta_credito",$ctacr),
                            array(":parametrizacion_anno",$anno),
