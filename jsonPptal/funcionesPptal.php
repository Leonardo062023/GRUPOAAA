...
function apropiacion($id_rubFue)
{
    ...
    $anno     = $_SESSION['anno'];
    $apropiacion_def = 0;
    $apropia = $con->Listar("SELECT   detComP.valor valorDetalleComprobantePPTAL, tipComP.tipooperacion  
    ...
    if (count($apropia) > 0) {
        if (count($apropia) == 1) {
            if (($apropia[0][1] == 2) || ($apropia[0][1] == 4) || ($apropia[0][1] == 1)) {
                $apropiacion_def += $apropia[0][0];
            } elseif ($apropia[0][1] == 3) {
                $apropiacion_def -= $apropia[0][0];
            }
        } else {
            for ($row = 0; $row < count($apropia); $row++) {
                if (($apropia[$row][1] == 2) || ($apropia[$row][1] == 4) || ($apropia[$row][1] == 1)) {
                    $apropiacion_def += $apropia[$row][0];
                } elseif ($row[1] == 3) {
                    $apropiacion_def -= $apropia[$row][0];
                }
            }
...
function balanceapropiacion($id)
{
    ...
    $ingresos = 0;
    $gastos = 0;
    $resultado = $con->Listar("SELECT det.valor, tipclap.id_unico
    ...
    for ($row = 0; $row < count($resultado); $row++) {
        if ($resultado[$row][1] == 6)
            $ingresos += $resultado[$row][0];
        elseif ($resultado[$row][1] == 7)
            $gastos += $resultado[$row][0];
    }
...
function numeroFactura($tipo, $anno)
{
    ...
    $numero     = "";
    $nanno      = anno($anno);
    $cons = $con->Listar("SELECT sigue_consecutivo FROM gp_tipo_factura WHERE id_unico = $tipo");
    $res  = $con->Listar("SELECT numero_inicial FROM `gp_resolucion_factura` WHERE tipo_factura = $tipo ORDER BY id_unico DESC LIMIT 1");
    if ($cons[0][0] == 1) {
        $fac = $con->Listar("SELECT * FROM gp_factura WHERE tipofactura = $tipo");
        if (count($fac) > 0) {
            $sql = $con->Listar("SELECT MAX(cast(numero_factura as unsigned))+1 FROM gp_factura where tipofactura = $tipo ");
            $numero = $sql[0][0];
        } else {
            if (count($res) > 0) {
                $numero = $res[0][0];
        ...
        $fac = $con->Listar("SELECT * FROM gp_factura WHERE tipofactura = $tipo AND parametrizacionanno = $anno");
        if (count($fac) > 0) {
            $sql = $con->Listar("SELECT REPLACE(MAX(numero_factura), MAX(cast(numero_factura as unsigned)),MAX(cast(numero_factura as unsigned))+1) FROM gp_factura where tipofactura = $tipo AND parametrizacionanno = $anno");
            $numero = $sql[0][0];
        } else {
            if (count($res) > 0) {
                $numero = $res[0][0];
...
{
    global $con;
    $row = $con->Listar("SELECT anno FROM gf_parametrizacion_anno WHERE id_unico = $id");
    $anno = $row[0][0];
    return ($anno);
}
...
function reconstruirComprobantesFactura($id_factura)
{
    ...
    $fc = $con->Listar("SELECT f.*, tc.id_unico as cnt, tcp.id_unico as pptal, 
        tc.tipo_comp_hom as csc , tf.tipo_movimiento as almacen
        FROM gp_factura f 
    ...
    if (!empty($cnt) || !empty($pptal)) {
        #** Buscar Retenciones 
        $tr = 0;
        $rt = $con->Listar("SELECT r.id_unico, c.id_unico, c.naturaleza,
            r.valorretencion , cr.id_unico, cc.id_unico, cc.naturaleza 
            FROM gf_retencion r 
        ...
        for ($i = 0; $i < count($rt); $i++) {
            if ($rt[$i][4] != 1) {
                ...
                $sql_dato = array(
                    ...
                    array(":valor", ($vrcr)),
                    array(":cuenta", $rt[$i][5]),
                    array(":naturaleza", $rt[$i][6]),
                    array(":tercero", $tercero),
                    array(":centrocosto", $centrocosto),
                    array(":detallecomprobantepptal", NULL),
                );
                $resp = $con->InAcEl($sql_cons, $sql_dato);
            }
            $vr  = $rt[$i][3];
        ...
        #** Buscar Detalles Factura ***#
        $rowd = $con->Listar("SELECT id_unico, factura, concepto_tarifa, valor, 
            cantidad, iva, impoconsumo, ajuste_peso, 
            valor_total_ajustado , valor_descuento 
            FROM 
              gp_detalle_factura WHERE factura =$id_factura ");
        for ($i = 0; $i < count($rowd); $i++) {
            ...
            LEFT JOIN gf_cuenta cds ON cds.id_unico = crc.cuenta_descuento 
            ...
            if (count($sqlc) > 0) {
                ...
                if (!empty($pptal)) {
                    ...
                    $utl = $con->Listar("SELECT MAX(id_unico) FROM gf_detalle_comprobante_pptal WHERE comprobantepptal = $pptal");
                    $id_dpptal = $utl[0][0];
                }
                if (!empty($cnt)) {
                    if (!empty($cuenta_debito) && !empty($cuenta_credito)) {
                        if ($descuento > 0) {
                            $valor_desc = $descuento;
                            if ($nat_des == 2) {
                                $valor_desc = $valor_desc * -1;
                            }
                            $sql_cons = "INSERT INTO `gf_detalle_comprobante` 
                                    ( `fecha`, `comprobante`,`valor`,
                            ...
                            $sql_dato = array(
                                array(":fecha", $fecha),
                                array(":comprobante", $cnt),
                                array(":valor", ($valor_desc)),
                                array(":cuenta", $cuenta_des),
                                array(":naturaleza", $nat_des),
                                array(":tercero", $tercero),
                                array(":centrocosto", $centrocosto),
                        ...
                        if ($descuento > 0) {
                            $valor_debito -= $descuento;
                        }
                        if ($nat_debito == 2) {
                            $valor_debito = $valor_debito * -1;
                        }
                        $sql_cons = "INSERT INTO `gf_detalle_comprobante` 
                                ( `fecha`, `comprobante`,`valor`,
...
#*** Eliminar Detalles Movimiento Por comprobante
function eliminardetallesMov($mov)
{
    global $con;
    #*** Eliminar Detalles Cnt
...
function eliminardetallesFactura($factura)
{
    global $con;
    ...
    #var_dump($resp);
    if (empty($resp)) {
        $mh = $con->Listar("SELECT mov_hotel FROM gp_factura WHERE id_unico = $factura");
        if (!empty($mh[0][0])) {
            $sql_cons = "UPDATE `gp_factura`
                SET `mov_hotel`=:mov_hotel 
                WHERE `id_unico`=:id_unico ";
            $sql_dato = array(
                array(":mov_hotel", NULL),
...
function idannoanterior($idannoa)
{
    global $con;
    $anno = $con->Listar("SELECT paan.id_unico FROM gf_parametrizacion_anno pa 
    LEFT JOIN gf_parametrizacion_anno paan ON paan.anno = (pa.anno-1) AND pa.compania = paan.compania 
    WHERE pa.id_unico =$idannoa");
    return $anno[0][0];
}
