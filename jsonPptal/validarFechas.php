    ...
    switch ($estruc) {
        ...
        case 1:
            $tipComPal = $_REQUEST['tipComPal'];
            $fecha = fechaC($_REQUEST['fecha']);
            ####VERIFICA SI HAY COMPROBANTES####
            $numCompr = $con->Listar("SELECT COUNT(*) 
                FROM gf_comprobante_pptal 
                WHERE tipocomprobante = $tipComPal AND parametrizacionanno = $anno");
            $numCompr = $numCompr[0][0];
            if ($numCompr > 0) {
                ##NUMERO MAYOR###
                $numCom = $con->Listar("SELECT MAX(numero)
                FROM gf_comprobante_pptal 
                WHERE tipocomprobante = $tipComPal  AND parametrizacionanno = $anno");
                $numCom = $numCom[0][0];
                ###FECHA NUMERO###
                $fechaComp = $con->Listar("SELECT fecha
                    FROM gf_comprobante_pptal 
                    WHERE tipocomprobante = $tipComPal AND numero = $numCom");


                $fechaComp = $fechaComp[0][0];
                $fecha_prev = ($fechaComp);

                if ($fecha >= $fecha_prev) {
                    $sumDias = $con->Listar("SELECT valor FROM gs_parametros_basicos WHERE nombre = 'Días Vencimiento Disponibilidad'");

                    if (count($sumDias) > 0) {
                        $sumarDias = $sumDias[0][0];
                    } else {
                        $sumarDias = 30;
                    }
        ...
        case 2:
            ...
            $fnm = $con->Listar("SELECT fecha FROM gf_comprobante_pptal WHERE 
                numero = $numM AND tipocomprobante =$tipComPal AND parametrizacionanno = $anno");
            #####BUSCAR LA FECHA DEL NUMERO MAYOR##
            $fnma = $con->Listar("SELECT fecha FROM gf_comprobante_pptal WHERE 
