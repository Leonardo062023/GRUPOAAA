...
switch ($estruc) {
    ...
    case 2:
        ...
        $rowPA = $con->Listar("SELECT anno 
                FROM gf_parametrizacion_anno 
                WHERE id_unico = $parametroAnno");
        $numero = $rowPA[0][0];

        $row = $con->Listar('SELECT MAX(numero) 
    ...
    case 5:
        ...
        echo $rf = $con->Listar("SELECT f.id_unico, f.nombre FROM gf_rubro_fuente rf
                LEFT JOIN gf_fuente f ON rf.fuente = f.id_unico
                WHERE rf.rubro = $rubro ");
        if (count($rf) > 0) {
            echo "<option value='" . $rf[0][0] . "'>" . ucwords(mb_strtolower($rf[0][1])) . "</option>";
            $fuente = $con->Listar("SELECT id_unico, nombre    
            FROM gf_fuente where id_unico !=$rf[0] AND parametrizacionanno = $anno");
            for ($rowFue = 0; $rowFue < count($fuente); $rowFue++) {
                echo "<option value='" . $fuente[$rowFue][0] . "'>" . ucwords(mb_strtolower($fuente[$rowFue][1])) . "</option>";
            }
        } else {
            $fuente = $con->Listar("SELECT id_unico, nombre    
            ...
            for ($row=0; $row < count($fuente); $row++) { 
                echo "<option value='". $fuente[$row][0] ."'>" . ucwords(mb_strtolower($fuente[$row][1])) . "</option>";
            }
        }
        break;
    ...
    case 37:
        $codigo = '';
        $pred   = $_REQUEST['padre'];
        $rowc   = $con->Listar("SELECT codi, LENGTH(codi) FROM gf_plan_inventario where  id_unico =$pred");
        $cod    = $rowc[0][0];
        $tm     = $rowc[0][1];
        $rowce  = $con->Listar("SELECT codi, length(codi), codi+1 FROM gf_plan_inventario where  codi like '$cod%' and id_unico !=$pred AND compania = $compania ORDER BY codi ASC");
        if (count($rowce) > 0) {
            $e = 0;
            $i = $rowce[0][1];
            $rowc = $con->Listar("SELECT codi, length(codi), codi+1 FROM gf_plan_inventario where  codi like '$cod%' and length(codi)='$i' AND compania = $compania ORDER BY codi DESC");
            $codigo = $rowc[0][2];
        } else {
            $codigo = $cod . '01';
        }
