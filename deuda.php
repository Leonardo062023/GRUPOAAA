...
HAVING (r.valor-r.valor_p)<100 and (r.valor-r.valor_p)>0
...
for ($i=0; $i <count($row) ; $i++) { 
	...
	$valor_i = $row[$i][5];
	$id_fact = $row[$i][6];
	$rowa = $con->Listar("SELECT * FROM gp_detalle_factura where factura = $id_fact and concepto_tarifa = 93");
	...
	if(empty($rowa[0][0])){
		...
		echo $sql = "UPDATE gp_detalle_factura SET valor = valor +$valor_i, valor_total_ajustado = valor_total_ajustado +$valor_i where id_unico = ".$rowa[0][0];
    	$resultadoP = $mysqli->query($sql);

    	$dr = $con->Listar("SELECT * FROM gp_detalle_pago where detalle_factura =".$rowa[0][0]);
    	echo "SELECT * FROM gp_detalle_pago where detalle_factura =".$rowa[0][0];
    	if(empty($dr[0][0])){
    		$sql_cons ="INSERT INTO `gp_detalle_pago`
		        ( `detalle_factura`, `valor`,
		        `iva`,`impoconsumo`,
	    	...
	    	echo $sql = "UPDATE gp_detalle_pago SET valor = valor +$valor_i where id_unico = ".$dr[0][0];
	    	$resultadoP = $mysqli->query($sql);
	    }
...
$sql = "UPDATE dbo_recaudos_febrero r set r.valor_p = (SELECT SUM(dp.valor) from gp_detalle_pago dp where dp.pago = r.id_pago)";
    $resultadoP = $mysqli->query($sql);
    echo 'FIN';*/
