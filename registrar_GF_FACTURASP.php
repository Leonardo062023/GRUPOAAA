...
list($id_factura,$numero_factura, $id_tipo_factura, $tipo_factura, $id_tercero, $tercero, 
    ...
    $vtrm, $idfp, $fp,$idmp, $mp, $tcb, $rc, $nr, $id_uvms, $usuario, $cuotas, $abono) = 
        array('', '', '', 'Tipo Factura','', 'Tercero',
    '', '', '', 'Centro Costo', '', '', 'Estado Factura', '', 'Vendedor', '', '', '', '', 0,0, 0, '','','','','','', '',0, '', '', '', '');
...
if(!empty($_REQUEST['factura'])){
    ...
    t.id_unico, IF(t.razonsocial IS NULL OR t.razonsocial ='', CONCAT_WS(' ', t.nombreuno, t.nombredos, t.apellidouno, t.apellidodos, ' - ',t.numeroidentificacion),CONCAT_WS(' - ',t.razonsocial, t.numeroidentificacion)) , 
        ...
        f.tipo_cambio, uvms.id_unico, uv.codigo_ruta, CONCAT_WS(' ', tuv.nombreuno, tuv.nombredos, tuv.apellidouno, tuv.apellidodos, tuv.razonsocial, tuv.numeroidentificacion), f.cuotas, f.abono 
    FROM 
            gp_factura f
    LEFT JOIN 
    ...
    LEFT JOIN 
            gp_unidad_vivienda_medidor_servicio uvms ON f.unidad_vivienda_servicio = uvms.id_unico 
    LEFT JOIN 
    ...
    LEFT JOIN 
            gf_tercero tuv ON uv.tercero = tuv.id_unico 

    WHERE MD5(f.id_unico)='".$_REQUEST['factura']."'");
    list($id_factura,$numero_factura, $id_tipo_factura, $tipo_factura, $id_tercero, $tercero, 
    $fecha_factura, $fecha_vencimiento, $id_cc, $centro_costo, $descripcion, $id_estado_f, 
    $estado_factura,$id_vendedor, $vendedor, $cufe,$idfp, $fp,$idmp, $mp, $tcb,$id_uvms, $usuario, $cuotas, $abono )= 
    array($df[0][0], $df[0][1], $df[0][2], $df[0][3],$df[0][4], $df[0][5],
    $df[0][6], $df[0][7], $df[0][8], $df[0][9], $df[0][10], $df[0][11], $df[0][12], 
    $df[0][13], $df[0][14],$df[0][15], $df[0][18], $df[0][19], $df[0][20],$df[0][21],$df[0][22], $df[0][23], $df[0][24].' - '.$df[0][25], $df[0][26], $df[0][27]  );
...
if($_REQUEST['t']==1){
    ...
    $rowtf = $con->Listar("SELECT id_unico, 
        CONCAT_WS(' - ',UPPER(prefijo), nombre ) 
        FROM gp_tipo_factura WHERE compania =$compania AND clase_factura = 2 ");
    $informes = 'informes/inf_com_rem.php?factura='.md5($id_factura);
}
...
$rowt = $con->Listar("SELECT t.id_unico, IF(t.razonsocial IS NULL OR t.razonsocial ='', 
    ...
    FROM gf_tercero t 
    WHERE t.compania = $compania $hrt ORDER BY numeroidentificacion = '9999999999' DESC LIMIT 20"); 
#Centro Costo
$hcc    = '';
$htmlcc = '';
if($id_cc!=''){
...
<html>
                    ...
                    <div style="margin-top: -7px; border:4px solid #020324; border-radius: 10px;" class="client-form col-sm-12 col-lg-12 col-md-12">
                        <form id="form" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data" action="javaScript:guardarF()">
                            ...
                            <div class="form-group">
                                <label for="sltUsuario" class="control-label col-sm-1 col-md-1 col-lg-1 text-right">Usuario:</label>
                                <div class="col-sm-2 col-md-2 col-lg-2">
                                   <select name="sltUsuario" id="sltUsuario" class="form-control"  title="Seleccione el tipo de factura"  >
                                        <?php 
                                        $htmlI ='';

                                        if(empty($_REQUEST['factura']) ){
                                            $htmlI = '<option value=""> Usuario </option>';
                                            $rowo  = $con->Listar("SELECT uvms.id_unico, uv.codigo_ruta, CONCAT_WS(' ', t.nombreuno, t.nombredos, t.apellidouno, t.apellidodos, t.razonsocial, t.numeroidentificacion)  FROM gp_unidad_vivienda uv 
                                            ...
                                            for ($o = 0; $o < count($rowo); $o++) {
                                                 $htmlI .= '<option value="'.$rowo[$o][0].'">'.$rowo[$o][1].'  - '.$rowo[$o][2].'</option>';
                                            }
                                        } else {
                                            $htmlI .= '<option value="'.$id_uvms.'">'.$usuario.'</option>';
                                        }
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-2 col-md-2 col-lg-2">
                                    ...
                                    if(!empty($idmp)){
                                        ...
                                        if(!empty($id_factura)){
                                            echo '<input type="radio" name="txtFormaP" id="txtFormaP" tittle="Forma Pago" value="1" style="margin-left:0px" checked="checked">Contado';
                                            echo '<input type="radio" name="txtFormaP" id="txtFormaP" tittle="Forma Pago" value="2" style="margin-left:10px">Crédito';
                                        } else {
                                            echo '<input type="radio" name="txtFormaP" id="txtFormaP" tittle="Forma Pago" value="1" checked="checked" style="margin-left:0px">Contado';
                                            echo '<input type="radio" name="txtFormaP" id="txtFormaP" tittle="Forma Pago" value="2" style="margin-left:10px">Crédito';
                            ...
                            <div class="form-group">
                                <?php if(!empty($tcb)){ ?>
                                    <div id="conversion" >
                                        <label class="cambio control-label col-sm-2 col-md-2 col-lg-2 text-right" for="tipoc" id="tipoc"><strong class="obligado">*</strong></label>
                                        <div class="col-sm-1 col-md-1 c col-l-1">
                                            <input type="checkbox" id="tipo_cb" name="tipo_cb" checked /><br/>
                                            <input type="hidden" name="tipo_cambio" id="tipo_cambio" value="<?= $tcb ?>">
                                        </div>
                                    </div>    
                                <?php } else { ?>
                                    <div id="conversion" style="display: none;">
                                        <label class="cambio control-label col-sm-2 col-md-2 col-lg-2 text-right" for="tipoc" id="tipoc"><strong class="obligado">*</strong></label>
                                        <div class="col-sm-1 col-md-1 c col-l-1">
                                            <input type="checkbox" id="tipo_cb" name="tipo_cb"/><br/>
                                ...
                                <?php } 
                                 if(!empty($idmp)){
                                    if($idmp==2) { ?>
                                        <label class="col-sm-1 col-md-1 col-lg-1 text-rigth control-label">N° Cuotas:</label>
                                        <div class="col-sm-2 col-md-2 col-lg-2">
                                            <input type="text" name="txtCuotas" id="txtCuotas" style="width: 100%;" class="form-control" placeholder="N° Cuotas"  value="<?= $cuotas; ?>">
                                        </div>
                                        <label class="col-sm-1 col-md-1 col-lg-1 text-rigth control-label">Valor Abono:</label>
                                        <div class="col-sm-2 col-md-2 col-lg-2">
                                            <input type="text" name="txtAbono" id="txtAbono" style="width: 100%;" class="form-control" placeholder="Valor Abono"  value="<?= $abono; ?>">
                                        </div>
                                    <?php } 
                                } else { ?>
                                    <div id="abonos" style="display: none">                                            
                                        ...
                                        <label class="col-sm-1 col-md-1 col-lg-1 text-rigth control-label">Valor Abono:</label>
                                        <div class="col-sm-2 col-md-2 col-lg-2">
                                            <input type="text" name="txtAbono" id="txtAbono" style="width: 100%;" class="form-control" placeholder="Valor Abono" disabled="true" value="<?= $abono; ?>">
                                        </div>
                                    </div>
                                <?php } ?>
                            </div>
                ...
                <div class="col-sm-8 col-md-8 col-lg-8 col-sm-2 col-md-2 col-lg-2">
                    ...
                    <?php if($nr>0){ ?>
                        <div class="col-sm-12 col-md-12 col-lg-12"  style="margin-top: -4px;">
                            <input type="hidden" name="numR" id="numR" value="<?=$nr;?>"/>
                    ...
                    <form name="form-detalle" id="form-detalle" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="javaScript:guardarDetalles()"  style="margin-top:-5px;">
                        ...
                        <div class="col-sm-1" style="margin-right:11px; margin-left:-30px;width:150px;">
                            <div class="form-group"  align="left">
                                ...
                                <input type="text" name="concepto" id="concepto" class="form-control"  title="Ingrese Concepto"  placeholder="Concepto" required style="display: inline; width: 150px" onchange="valorCambio();" autocomplete="off">
    ...
    <div class="modal fade" id="myModalEliminar" role="dialog" align="center" >
        ...
        <script>
            ...
            $('#s2id_autogen1_search').on("keydown", function(e) {
                ...
                $.ajax({
                    ...
                    success: function(data){
                        ...
                        console.log(data);
                         option = option+data;
                        $("#sltUsuario").html(data);
                    }
                }); 
            });
             $("#sltUsuario").change(function(){
                let form_data4 = {action: 60, uvms : $("#sltUsuario").val()};
                console.log('USUSARIOtercero');
                $.ajax({
                    type:"POST",
                    url:"jsonPptal/gf_facturaJson.php",
                    data:form_data4,
            ...
            $('input[type=radio][name=txtFormaP]').change(function() {
                if (this.value == '2') {
