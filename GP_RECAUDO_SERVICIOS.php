                            ...
                            <div class="form-group form-inline " style="margin-top: -5px;margin-left: -10px">
                                ...
                                <script>
                                     function cambiarfecha(){
                                         document.location='GP_RECAUDO_SERVICIOS.php?f='+$("#fecha").val();
                                     }
                                </script>
                            ...
                            <div class="table-responsive" style="margin-left: 5px; margin-right: 5px;">
                                    ...
                                    <tbody>
                                        <?php 
                                            ...
                                            if(!empty($_REQUEST['s'])){
                                                ...
                                                WHERE f.fecha_factura <='$fecha' AND uv.sector = ".$_REQUEST['s']." 
                                                AND f.tipofactura = 1 
                                                GROUP BY f.id_unico ORDER BY uvms.id_unico, t.id_unico, f.fecha_factura" );
                                            } else {
                                            $rowd = $con->Listar("SELECT f.id_unico, 
                                                f.numero_factura, 
                                                DATE_FORMAT(f.fecha_factura, '%d/%m/%Y'), 
                                                ...
                                                LEFT JOIN gp_unidad_vivienda uv ON uvs.unidad_vivienda = uv.id_unico 
                                                LEFT JOIN gp_predio1 p ON uv.predio = p.id_unico 
                                                WHERE f.fecha_factura <='$fecha'  AND f.tipofactura = 1 
                                                GROUP BY f.id_unico ORDER BY uvms.id_unico, t.id_unico, f.fecha_factura" );
                                            }
                                            $num =0;
                                            for ($i = 0; $i < count($rowd); $i++) {    
                                                ...
                                                if($saldo>0){
                                                    ...
                                                    echo '<input name="seleccion'.$num.'" id="seleccion'.$num.'" type="checkbox" onchange="cambiarV('.$num.')">';
                                                    echo '<input name="factura'.$num.'" id="factura'.$num.'" type="hidden" value="'.$rowd[$i][0].'">';
                                                    echo '<input name="saldo'.$num.'" id="saldo'.$num.'" type="hidden" value="'.$saldo.'">';
                                                    echo '</td>';
                                                    ...
                                                    echo '<input name="valor_r'.$num.'" id="valor_r'.$num.'" value = "'.number_format($saldo,0,'.',',').'" '
                                                            . 'class="col-sm-2 form-control" title="Seleccione Valor" required '
                                                            . 'title="Ingrese el valor" style=" width: 95%"  '
                                                            . 'onkeyup="valor('.$num.')" onchange="valor_c('.$num.')"/>';
                                                    echo '<input type="hidden" name="valor_c'.$num.'" id="valor_c'.$num.'" value="'.$saldo.'">';
                                                    echo '<input type="hidden" name="valor_rr'.$num.'" id="valor_rr'.$num.'" value="'.$saldo.'">';
                                                    echo '</td>';
                                                    echo '</tr>';
                                                    $num +=1;
                                                }
                                ...
                                function guardard(){
                                    ...
                                    var formData = new FormData($("#form")[0]);  
                                        ...
                                        $.ajax({
                                        type: 'POST',
                                        url: "jsonServicios/gp_facturacionServiciosJson.php?action=3&f="+facturas_s+"&num="+$("#num").val(),
                                        data:formData,
                                        contentType: false,
                                        processData: false,
                                        ...
                                        { 
                                            ...
                                            if(rta !=0){
                                                ...
                                                $("#Aceptar").click(function(){
                                                    window.location='GP_RECAUDO_SERVICIOS.php';
                                                })
                                            } else {
