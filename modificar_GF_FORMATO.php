...
switch ($_SESSION['conexion']) {
    case '1':
      $rowF = $con->Listar("SELECT fot.id_unico, fot.nombre, fot.version, date_format(fot.fechaversion,'%d/%m/%Y'),
      fot.descripcion, fot.esCheque, fot.codigo_calidad, p.id_unico, p.nombre FROM 	gf_formato fot 
      LEFT JOIN gf_plantilla p ON fot.plantilla = p.id_unico WHERE	(fot.id_unico)='$idF'");
    break;
    case '2':
      $rowF = $con->Listar("SELECT fot.id_unico, fot.nombre, fot.version,  to_char(fot.fechaversion,'DD-MM-YYYY'),
      fot.descripcion, fot.esCheque, fot.codigo_calidad, p.id_unico, p.nombre FROM 	gf_formato fot 
      LEFT JOIN gf_plantilla p ON fot.plantilla = p.id_unico WHERE	(fot.id_unico)='$idF'");
  break;
  }
    ...
    <div class="container-fluid text-left">
        <div class="row content">
            ...
            <div class="col-sm-8 text-left">
                ...
                <h5 id="forma-titulo3a" align="center" style="width:92%; display:inline-block; margin-bottom: 10px; margin-right: 4px; margin-left: 4px; margin-top:-5px;  background-color: #0e315a; color: white; border-radius: 5px">Formato: <?php echo ucwords(mb_strtolower($rowF[$e][1]))?></h5>
                <div class="client-form contenedorForma" style="margin-top: -7px">
                <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="javascript:modificar()">
                    
                    <?php for ($e=0; $e <count($rowF) ; $e++) {?>
                        ...
                        <input type="hidden" name="txtId" id="txtId" class="hidden" value="<?php echo $rowF[$e][0]; ?>">
                        <div class="form-group" style="margin-top: 5px;">
                            <label for="txtNombre" class="col-sm-5 control-label">
                                <strong class="obligado">*</strong>Nombre:
                            </label>
                            <input type="text" name="txtNombre" id="txtNombre" class="form-control" maxlength="100" title="Ingresa el nombre" placeholder="Nombre" onkeypress="return txtValida(event, 'num_car')" required="" value="<?php echo ucwords(mb_strtolower($rowF[$e][1])); ?>">
                        </div>
                        <div class="form-group" style="margin-top: -10px;">
                            <label for="txtCodigo" class="col-sm-5 control-label">
                                <strong class="obligado"></strong>Código Calidad:
                            </label>
                            <input type="text" name="txtCodigo" id="txtCodigo" class="form-control" maxlength="100" title="Ingresa el Código de Calidad" placeholder="Código de Calidad" onkeypress="return txtValida(event, 'num_car')" value="<?php echo (($rowF[$e][6])); ?>">
                        </div>
                        <div class="form-group" style="margin-top: -10px">
                            <label for="txtVersion" class="col-sm-5 control-label">
                                Versión:
                            </label>
                            <input type="text" name="txtVersion" id="txtVersion" class="form-control" maxlength="100" title="Ingrese la versión del formato" placeholder="Versión" onkeypress="return txtValida(event, 'sin_espcio')" value="<?php echo ucwords(mb_strtolower($rowF[$e][2])); ?>">
                        </div>
                        <div class="form-group" style="margin-top: -10px">
                            <label for="txtFechaVersion" class="col-sm-5 control-label">
                                <strong class="obligado">*</strong>Fecha Versión:
                            </label>
                            <input type="text" name="txtFechaVersion" id="txtFechaVersion" class="form-control" title="Seleccione la fecha" placeholder="Fecha Versión" required value="<?php echo $rowF[$e][3]; ?>">
                        </div>
                        ...
                        <div class="form-group" style="margin-top: -5px">
                            ...
                            <textarea type="text" name="txtDescripcion" id="txtDescripcion" class="form-control" title="Ingrese descripción del formato" placeholder="Descripción"><?php echo ucwords(mb_strtolower($rowF[$e][4])); ?></textarea>
                        </div>
                        <div class="form-group" style="margin-top: -10px">
                            ...
                            <select name="plantilla" id="plantilla" class="select2_single form-control" title="Seleccione Plantilla">
                                ...
                                if (empty($rowF[$e][7])) {
                                    ...
                                    echo '<option value="'.$rowF[$e][7].'">'.$rowF[$e][8].'</option>';
                                    echo '<option value=""> - </option>';
                                    $row = $con->Listar("SELECT * FROM gf_plantilla WHERE id_unico != ".$rowF[$e][7]);
                                }
...
<script>
    ...
    function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/procesos_GF_FORMATO.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
                                success: function(response)
                                {
                                    ...
                                    if(response==1){
                                        ...
                                        $("#modalMensajes").modal("show");
                                            ...
                                            document.location='listar_GF_FORMATO.php';
