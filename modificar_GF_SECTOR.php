...
if (isset($_GET["id_sector"])) {
  $id_sector = base64_decode(($_REQUEST["id_sector"]));
  //Query o sql de consulta     
  $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_sector  WHERE Id_Unico ='$id_sector'");
}
...
<div class="container-fluid text-center">
  <div class="row content">
    ...
    <div class="col-sm-10 text-left">
      <?php if ($action == 1) { ?>
        <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;">Modificar Sector</h2>
      <?php }
      ...
      <div style="border: 4px solid #020324; border-radius: 10px; margin-left: 4px; margin-right: 4px;" class="client-form">
        ...
        <form name="form" id="form" class="form-horizontal" method="POST" enctype="multipart/form-data" action="">
          ...
          <div class="form-group" style="margin-top: -10px;">
            ...
            <?php if ($action == 1) { ?>
              <input type="text" name="nombre" id="nombre" class="form-control" maxlength="150" title="Ingrese el nombre" onkeypress="return txtValida(event,'car')" placeholder="Nombre" value="<?php echo ucwords(strtolower($row[0][1])); ?>" required>
            <?php }
...
<script>
  function modificar() {
    ...
    $.ajax({
      type: 'POST',
      url: "Json/modificarSectorJson.php?action=3",
      data: formData,
      contentType: false,
      processData: false,
  ...
  function agregar() {
    ...
    $.ajax({
      type: 'POST',
      url: "Json/modificarSectorJson.php?action=2",
      data: formData,
      contentType: false,
      processData: false,
      success: function(response) {
