...
$sqlP="SELECT (dtm.valor+dtm.iva) as valort ,pro.descripcion
             FROM      gf_producto_especificacion prdes
             LEFT JOIN gf_ficha_inventario     fin ON fin.id_unico  = prdes.fichainventario
             left join gf_elemento_ficha       elm ON elm.id_unico  = fin.elementoficha
             LEFT JOIN gf_movimiento_producto movp ON movp.producto = prdes.producto
             LEFT JOIN gf_detalle_movimiento dtm ON movp.detallemovimiento=dtm.id_unico
             LEFT JOIN gf_producto pro ON movp.producto=pro.id_unico
             WHERE     pro.id_unico =$producto
             GROUP BY  prdes.producto
             ORDER BY  prdes.producto,elm.id_unico ASC";
$resultP = $mysqli->query($sqlP);
while($row=$resultP->fetch_row()){ ?>
    <label class="control-label col-lg-4 col-sm-4"><strong class="obligado">*</strong>Valor:</label>
    <input style="width: 280px;" onkeypress="return txtValida(event,'num')" type="text" name="txtValor" value="<?php echo $row[0];?>" id="txtValor" class="form-control col-sm-1 input-sm" maxlength="150" title="Ingrese el valor" placeholder="Valor" required >
    <label class="control-label col-lg-4 col-sm-4">Descripción:</label>
    <textarea class="form-control col-lg-1 col-sm-1 area" style="margin-top:-1px;height:40px;width: 280px;" rows="2" onkeypress="return txtValida(event,'num_car');" maxlength="500" minlength="0" name="txtDescripcion" id="txtDescripcion" title="Descripción del producto"><?php echo $row[1]; ?></textarea>
<?php
if($anterior==0){
                ...
                <div style="margin-top:-20px;" class="client-form">
                    ...
                    <div class="form-group form-inline" style="margin-top:5px;">
                        ...
                        if (!empty($_POST['ficha'])) {
                            $vFicha = $_POST['ficha'];
                            $sqlF2 = "select dtm.cantidad,(dtm.valor+dtm.iva) as valor,dtm.id_unico from gf_plan_inventario pln left join gf_detalle_movimiento dtm on dtm.planmovimiento=pln.id_unico where pln.ficha = $ficha";
