...
switch ($case) {
    case 1:
        ...
        $bi = $con->Listar("SELECT  tp.obligacionafectacion
            FROM gf_comprobante_pptal cp 
            LEFT JOIN gf_tipo_comprobante_pptal tp ON cp.tipocomprobante = tp.id_unico 
            WHERE cp.id_unico = $cuentaxpagar ");
        if($bi[0][0]==2){
            $query = "SELECT cn.id_unico
            FROM gf_comprobante_pptal cp 
            LEFT JOIN gf_tipo_comprobante_pptal tp ON cp.tipocomprobante = tp.id_unico 
            LEFT JOIN gf_tipo_comprobante tc ON tc.comprobante_pptal = tp.id_unico 
            LEFT JOIN gf_comprobante_cnt cn ON cp.numero = cn.numero AND tc.id_unico = cn.tipocomprobante 
            WHERE cp.id_unico = $cuentaxpagar";
        } else {
            $query = "SELECT
                    dc.comprobante
