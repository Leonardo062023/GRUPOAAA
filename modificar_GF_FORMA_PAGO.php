...
switch ($_SESSION['conexion']) {
  case '1':
    $row = $con->Listar("SELECT Id_Unico, Nombre, dispersion FROM gf_forma_pago  WHERE (Id_Unico) ='$id_forma'");
  break;
  case '2':
    $row = $con->Listar("SELECT Id_Unico, Nombre, dispersion FROM gf_forma_pago  WHERE (Id_Unico) ='$id_forma'");
break;
...
?>
  <title>Modificar Forma Pago</title>
</head>
...
<!-- Llamado al menú del formulario -->   
    ...
    <div class="col-sm-10 text-left">
      ...
      <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;">Modificar Forma Pago</h2>
          ...
          <p align="center" style="margin-bottom: 25px; margin-top: 25px; margin-left: 30px; font-size: 80%">Los campos marcados con <strong style="color:#03C1FB;">*</strong> son obligatorios.</p>
            ...
            <div class="form-group" style="margin-top: -10px;">
              <label for="dispersion" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Dispersión:</label>
                <?php if($row[$i][2]==1){ 
                    echo '<input type="radio" name="dispersion" id="dispersion" value="1" checked="checked">Sí';
                    echo '<input type="radio" name="dispersion" id="dispersion" value="2" >No';
                } else {
                    echo '<input type="radio" name="dispersion" id="dispersion" value="1" >Sí';
                    echo '<input type="radio" name="dispersion" id="dispersion" value="2" checked="checked">No';
                } ?>
            </div>
            <div class="form-group" style="margin-top: 10px;">
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificarFormaPagoJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
