...
?>
<style>
    #tabla1 table.dataTable thead th,
    table.dataTable thead td {
        padding: 1px 18px;
        font-size: 10px
    }

    #tabla1 table.dataTable tbody td,
...
<div class="modal fade movi1" id="mdlDetalleMovimiento" role="dialog" align="center" aria-labelledby="mdlDetalleMovimiento" aria-hidden="true">
    <div class="modal-dialog" style="height:600px;width:1000px">
        <div class="modal-content">
            ...
            $con = new ConexionPDO();
            @session_start();
            $idcomprobantedetalle = $_REQUEST['id'];
            if (!empty($_REQUEST['valor'])) {
                if ($_REQUEST['valor'] < 0) {
                    $valorD = substr($_REQUEST['valor'], '1');
                } else {
                    $valorD = $_REQUEST['valor'];
                }
            }
            ...
            <div id="forma-modal" class="modal-header">
                ...
                <div class="col-sm-offset-11" style="margin-top:-30px;margin-right: -45px">
                    <button type="button" id="btnCerrarModalMov" class="btn btn-xs" style="color: #000;" data-dismiss="modal">
                                ...
                                <div class="form-group form-inline" style="width: 100%;">
                                    <div class="form-group form-inline col-sm-4">
                                        ...
                                        <select required="required" name="sltTipoDocumento" id="sltTipoDocumento" class="select2_single form-control" style="width:130px;height:30%" title="Seleccione tipo documento" required>
                                            ...
                                            $result10 = $con->Listar("SELECT id_unico,nombre FROM gf_tipo_documento WHERE compania = $compania");
                                            for ($fila10 = 0; $fila10 < count($result10); $fila10++) { ?>
                                                <option value="<?php echo $result10[$fila10[0]] ?>"><?php echo $result10[$fila10][1] ?></option>
                                            <?php }
                                            ?>
                                        </select>
                                    </div>
                                                ...
                                                success: function(data) {
                                                    if (data == 1) {
                                                        ...
                                                        $("#btnCheque").click(function() {
                                                            window.open('formatoCheque.php?idDP=<?php echo base64_encode($idcomprobantedetalle) ?>&tipo=' + tipoD);
                                                        });
                                                    } else {
                                    ...
                                    <a onclick="return guardarDetalleMovimiento(<?php echo $idcomprobantedetalle ?>)" class="btn btn-primary sombra" style="margin-top:1px; margin-left:-50px">
                        ...
                        <div class="table-responsive contTabla">
                            <table id="tabla1" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%" style="">
                                ...
                                <tbody>
                                    <?php
                                    if (!empty($_REQUEST['almacen'])) {
                                        $result11 = $con->Listar("SELECT dtm.id_unico,
                                                dtm.tipodocumento,
                                                tpd.id_unico,
                                                tpd.nombre,
                                                dtm.numero,
                                    ...
                                    for ($row11 = 0; $row11 < count($result11); $row11++) { ?>
                                        <tr>
                                            <td class="campos oculto">
                                                <?php echo $result11[$row11][0]; ?>
                                            </td>

                                            <td class="campos">
                                                <a href="#<?php echo $result11[$row11][0]; ?>" onclick="javascript:eliminarDetalleComprobanteMov(<?php echo $result11[$row11][0] . ',' . "'" . $result11[$row11][7] . "'"; ?>)" title="Eliminar">
                                                    <li class="glyphicon glyphicon-trash"></li>
                                                </a>
                                                <a href="#<?php echo $result11[$row11][0]; ?>" title="Modificar" id="mod" onclick="javascript:modificarDetalleMov(<?php echo $result11[$row11][0]; ?>);javascript:cargarFecha(<?php echo $result11[$row11][0]; ?>);javascript:crearNuevoMov(<?php echo $result11[$row11][0]; ?>)">
                                                    <li class="glyphicon glyphicon-edit"></li>
                                                </a>
                                            </td>
                                            <td class="campos">
                                                <?php echo '<label class="valorLabel" style="font-weight:normal" id="lbltipodocumento' . $result11[$row11][0] . '">' . (ucwords(strtolower($result11[$row11][3]))) . '</label>'; ?>
                                                <select style="display: none;padding:2px;height:19px" class="col-sm-12 campoD" id="slttipodocumento<?php echo $result11[$row11][0]; ?>">
                                                    <?php
                                                    echo '<option value="' . $result11[$row11][2] . '">' . (ucwords(strtolower($result11[$row11][3]))) . '</option>';
                                                    $sql12 = "SELECT id_unico,nombre FROM gf_tipo_documento WHERE id_unico!=$result11[$row11][2]";
                                                ...
                                                <script>
                                                    $("#slttipodocumento<?php echo $result11[$row11][0]; ?>").change(function() {
                                                        var documento = document.getElementById('slttipodocumento<?php echo $result11[$row11][0]; ?>').value;
                                            ...
                                            <td class="campos text-center">
                                                <div id="docD<?php echo $row11[0] ?>" name="docD<?php echo $row11[0] ?>" style="display:inline">
                                                    <?php if (empty($row11[7])) {
                                                        echo '<i>No hay documento</i>';
                                                    } else { ?>
