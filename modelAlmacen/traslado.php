...
class traslado{
    ...
    public function get_dept ($dept) {
        try {
          ...
          $compania = $_SESSION['compania'];
            $sql = "SELECT dtp.id_unico, CONCAT(dtp.sigla,' ' ,dtp.nombre) FROM gf_dependencia dtp WHERE md5(dtp.id_unico) = '$dept' AND compania = $compania AND (movimiento =0 OR activa = 0) ";
    ...
    public function get_allD ($compania, $orden = NULL){
        try {
            $sql = "SELECT dtp.id_unico, CONCAT(dtp.sigla,' ' ,dtp.nombre) FROM gf_dependencia dtp WHERE dtp.compania = $compania AND dtp.tipodependencia != 1 AND (movimiento =0 OR activa = 0)  ORDER BY dtp.nombre $orden";
            $res = $this->mysqli->query($sql);
    ...
    public function obtnerProductos(){
        try {
            @session_start();
            $compania = $_SESSION['compania'];
            $sql = "SELECT DISTINCT mp.producto FROM gf_movimiento_producto mp 
