...
class baja{
    ...
    public function obtnerDep(){
        try {
            ...
            $sql = "SELECT DISTINCT dep.id_unico, UPPER(dep.sigla), dep.nombre FROM gf_dependencia dep WHERE dep.tipodependencia = 2 
    ...
    public function obtnerDepDif($id_unico){
        try {
            $compania = $_SESSION['compania'];
            $sql = "SELECT dep.id_unico, UPPER(dep.sigla), dep.nombre FROM gf_dependencia dep WHERE md5(dep.id_unico) != '$id_unico' AND tipodependencia = 1 and compania = $compania";
            $res = $this->mysqli->query($sql);
            $row = $res->fetch_all(MYSQLI_NUM);
            return $row;
            $this->mysqli->close();
    ...
    public function obtnerProductos(){
        try {
            ...
            $sql = "SELECT DISTINCT mpr.producto
                    ...
                    LEFT JOIN       gf_detalle_movimiento dm ON mpr.detallemovimiento = dm.id_unico 
                    LEFT JOIN       gf_movimiento m ON dm.movimiento = m.id_unico 
                    WHERE           (pro.baja IS NULL OR pro.baja = 0) AND m.compania = $compania";
            $res = $this->mysqli->query($sql);
            $row = $res->fetch_all(MYSQLI_NUM);
            return $row;
            $this->mysqli->close();
    ...
    public function generarConsecutivo($tipo){
        try {
            $anno = $_SESSION['anno'];
            $xxx = 0;
            $sql = "SELECT MAX(numero) FROM gf_movimiento WHERE tipomovimiento = $tipo AND parametrizacionanno = $anno ORDER BY id_unico DESC";
            $res = $this->mysqli->query($sql);
            $row = $res->fetch_row();
            if(empty($row[0])){
                $xxx = date('Y').'000001';
    ...
    public function obtnerBaja($id_unico){
        try {
            $sql = "SELECT mov.id_unico as id,
                           ...
                           ) as doc, mov.observaciones as descripcion , mov.fecha as fechac
    ...
    public function obtnerDetallesBaja($mov){
        try {
            $sql = "SELECT    dtm.id_unico, dtm.valor, concat_ws(' ',pln.codi, pln.nombre) as producto, pro.descripcion, pEs.valor, pro.id_unico, dtm.iva 
    ...
    public function obtener_centro($nombre, $anno){
        try {
            $sql = "SELECT id_unico FROM gf_centro_costo WHERE nombre = '$nombre' and parametrizacionanno = $anno";
            $res = $this->mysqli->query($sql);
