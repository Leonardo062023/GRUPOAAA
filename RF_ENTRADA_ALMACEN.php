...
$tipoDocSoporte = ""; $numDocSoporte     = ""; $fuente = ""; $sumImpo =0;
$iddMov =0; $fecha_bd = date('Y-m-d'); $cierre = 0;
if(!empty($_GET['movimiento'])){
    ...
    $sql = "SELECT  mv.tipomovimiento, mv.numero, DATE_FORMAT(mv.fecha,'%d/%m/%Y'), mv.centrocosto, mv.proyecto, mv.dependencia,
                    ...
                    mv.tipo_doc_sop, mv.numero_doc_sop, mv.id_unico, mv.fuente , mv.fecha 
    ...
    $tipoDocSoporte = $rowPrimaria[17]; $numDocSoporte     = $rowPrimaria[18]; $fuente = $rowPrimaria[20];
    $fecha_bd =$rowPrimaria[21];

    //var_dump($rowA[15]);
    //var_dump(empty($rowA[15]));
    if(empty($rowPrimaria[16])){
        $porcIva = 0;
        $iva      =0;
    } else {
        $porcIva = $rowPrimaria[16];
        $iva     = $rowPrimaria[16];
    }
...
if(!empty($_GET['asociado'])) {
    ...
    $observaciones     = $rowA[13]; $idaso         = $rowA[14]; $id_aso       = $rowA[14]; $iva         = $rowA[15];  $tercero     = $rowA[16];
    ...
    <div class="container-fluid text-center">
        <div class="row content">
                ...
                <div class="client-form contenedorForma">
                    <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="controller/controllerGFEntradaAlmacen.php?action=insert" onsubmit="return validar()" style="margin-bottom: -30px">
                        ...
                        <div class="form-group" style="margin-top:-15px">
                            ...
                            <select name="sltDependencia" id="sltDependencia" class="col-lg-1 col-md-1 col-sm-1 form-control select2" style="width:10%;" title="Seleccione dependecia " required>
                                ...
                                if (!empty($dependencia)) {
                                    ...
                                    $sql10 = "SELECT DISTINCT id_unico,nombre FROM gf_dependencia WHERE id_unico != $dependencia AND compania = $compania AND tipodependencia = 1 AND (movimiento =0 OR activa = 0) ";
                                    ...
                                    if(!empty($_GET['asociado'])){
                                        $sql11 = "SELECT DISTINCT id_unico,nombre FROM gf_dependencia WHERE compania = $compania AND tipodependencia = 1 AND (movimiento =0 OR activa = 0) ";
                        ...
                        <div class="form-inline form-group" style="margin-top:-15px">
                            ...
                            <label for="sltFuente" class="col-lg-1 col-md-1 col-sm-1 control-label"><strong class="obligado"></strong>Fuente:</label>
                            <select name="sltFuente" id="sltFuente" title="Seleccione Fuente" style="width: 10%" class="col-lg-1 col-md-1 col-sm-1 form-control select2" >
                                <?php
                                if(empty($fuente)) {
                                    echo "<option value=\"\">Fuente</option>";
                                    $sqlFn = "SELECT id_unico, nombre FROM gf_fuente where parametrizacionanno = $param ORDER BY nombre ASC";
                                    $sqlFn = $mysqli->query($sqlFn);
                                    while($rowFn = mysqli_fetch_row($sqlFn)) {
                                        echo "<option value=\"".$rowFn[0]."\">".ucwords(mb_strtolower($rowFn[1]))."</option>";
                                    }
                                }else{
                                    $sqlTD = "SELECT id_unico, nombre FROM gf_fuente where id_unico=$fuente";
                                    $resultTD = $mysqli->query($sqlTD);
                                    $rowTD = mysqli_fetch_row($resultTD);
                                    echo "<option value=\"".$rowTD[0]."\">".ucwords(mb_strtolower($rowTD[1]))."</option>";
                                    $sqlFn = "SELECT id_unico, nombre FROM gf_fuente where parametrizacionanno = $param AND id_unico != $fuente";
                                    $sqlFn = $mysqli->query($sqlFn);
                                    while($rowFn = mysqli_fetch_row($sqlFn)) {
                                        echo "<option value=\"".$rowFn[0]."\">".ucwords(mb_strtolower($rowFn[1]))."</option>";
                                    }
                        ...
                        <div class="form-inline form-group" style="margin-top:-25px;padding: 10px;">
                            ...
                            <a id="btnNuevo" title="Ingresar nuevo" class="btn btn-primary shadow glyphicon glyphicon-plus text-center nuevo" onclick="javascript:nuevo()" style="margin-left: 5px;"></a>
                            ...
                            <div style=" margin-top: 8px;margin-bottom: 10px;">
                                ...
                                #buscar si existe comprobante asociado

                                ?>
                                <a id="btnVerC" title="Ver Comprobante Contable" class="btn btn-primary shadow glyphicon glyphicon-eye-open" disabled="true"></a>
                                <input type="hidden" name="id_comprobante_cnt" id="id_comprobante_cnt">
                                <a id="btnCrearC" title="Crear Comprobante Contable" class="btn btn-primary shadow glyphicon glyphicon-retweet" disabled="true"></a>
                                <a id="btnSubir" title="Subir Detalles por Archivo Plano" class="btn btn-primary shadow glyphicon glyphicon-upload" disabled="true"></a>
                            </div>
                            <?php }?>
                        </div>
                    </form>
                ...
                <form name="form" id="formD" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="controller/controllerGFMovimiento.php?action=insert_detail">
                    <div class="form-group">
                        ...
                        <div class="col-sm-2 col-md-2 col-lg-2">
                            <span class="has-float-label">
                                <input type="text" name="txtCantidad" class="form-control" id="txtCantidad" title="Cantidad" onkeypress="return txtValida(event, 'dec', 'txtCantidad', '2');" maxlength="10" placeholder="Cantidad" style="padding:2px;font-size: 10px;"/>
                                <label for="txtCantidad">Cantidad</label>
                            </span>
                        </div>
                        <div class="col-sm-2 col-md-2 col-lg-2">
                            <span class="has-float-label">
                                <input type="text" name="txtValor" id="txtValor" class="form-control col-lg-1 col-sm-1" title="Valor aproximado" onkeypress="return txtValida(event, 'dec', 'txtValor', '2');" maxlength="50" placeholder="Valor" style="padding:2px;font-size: 10px;" required="" />
                                <label for="txtValor">Valor Unitario</label>
                            </span>
                        </div>
                        <div class="col-sm-1 col-md-1 col-lg-1">
                            <span class="has-float-label">
                                <input type="text" name="txtValorIva" id="txtValorIva" class="form-control col-lg-1 col-sm-1" title="Iva" onkeypress="return txtValida(event,'num');" maxlength="50" placeholder="Iva" style="padding:2px;font-size: 10px;"  required="" />
                                <label for="txtValorIva">Iva</label>
                            </span>
                        </div>
                        <div class="col-sm-1 col-md-1 col-lg-1">
                            <span class="has-float-label">
                                <input type="text" name="txtValorImpo" id="txtValorImpo" class="form-control col-lg-1 col-sm-1" title="ImpoConsumo" onkeypress="return txtValida(event,'num');" maxlength="50" placeholder="Impo" style="padding:2px;font-size: 10px;"  />
                                <label for="txtValorImpo">Impo</label>
                            </span>
                        </div>
                        <div class="col-sm-1 col-md-1 col-lg-1">
                            <span class="has-float-label">
                                <input type="text" name="txtAjuste" id="txtAjuste"  class="form-control col-lg-1 col-sm-1" title="Ajuste"  maxlength="50" placeholder="Ajuste" style="padding:2px;font-size: 10px;" required="" value="0"/>
            ...
            if(!empty($_GET['movimiento'])) {
                ...
                echo "\n\t<script>";
                echo "$(\"#btnNuevo, #btnModificar,#btnOrden, #btnImprimir,#btnVerC,#btnCrearC,#btnSubir, #btnGuardarDetalle\").attr('disabled',true);";
                echo "$(\"#btnNuevo, #btnModificar,#btnOrden, #btnImprimir,#btnVerC,#btnCrearC,#btnSubir, #btnGuardarDetalle\").removeAttr('onclick');";
                echo " \n\t</script>";
            }
                    ...
                    <table id="tablaMovimiento" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
                        ...
                        <tbody>
                                ...
                                while ($row2 = mysqli_fetch_row($resultado2)) {
                                    ...
                                    if(empty($af[0][0])){
                                        if($cierre == 0){
                                            ...
                                            $html .= "\n\t\t\t<a href=\"#".$row2[0]."\" title=\"Modificar\" class=\"modificar\" onclick=\"modificar(".$row2[0].");\"><i class=\"glyphicon glyphicon-edit\"></i></a>";
                                        }
                                    }
                                    $html .= "\n\t\t</td>";
                                    ...
                                    $sumC+=$row2[2];
                                    $html .= "\n\t\t\t\t<label class=\"valorLabel\" style=\"font-weight:normal\" id=\"lblCantidad".$row2[0]."\">".number_format($row2[2], 2)."</label>";
                                    ...
                                    $html .= "\n\t\t\t<td class=\"text-right campos\">";
                                    $sumImpo +=$row2[10];
                                    $html .= "\n\t\t\t\t<label class=\"valorLabel\" style=\"font-weight:normal\" id=\"lblIva".$row2[0]."\">".number_format($row2[10],2,'.',',')."</label>";
                                    $html .= "\n\t\t\t</td>";
                                    $html .= "\n\t\t\t<td style=\"height:10px;font-size:10px\"class=\"text-right campos\">";
                                    $total = ($row2[3]+$row2[6]+$row2[10])*$row2[2];
                                    $totalmov+=$total;
                                    ...
                                    if($re_->num_rows > 0){
                                        ...
                                        $sqD = "SELECT ficha FROM gf_plan_inventario WHERE id_unico = $row2[1]";
                            ...
                            if(!empty($_GET['asociado'])){
                                $html = "";
                                $aso  = $_GET['asociado'];
                                $sql_d = "SELECT  dtm.id_unico, dtm.planmovimiento, dtm.cantidad, dtm.valor, pl.id_unico, pl.nombre, dtm.iva , pl.codi, dtm.impoconsumo 
                                        FROM      gf_detalle_movimiento dtm
                                        LEFT JOIN gf_movimiento mv ON dtm.movimiento = mv.id_unico
                                        LEFT JOIN gf_plan_inventario pl ON dtm.planmovimiento = pl.id_unico
                                ...
                                foreach ($row_d as $rowa) {
                                    ...
                                    $idPlan=$rowa[1];
                                    $iv = $con->Listar("SELECT 	iva_descontable FROM gf_plan_inventario WHERE id_unico = $idPlan");
                                    $iva_descontable   = $iv[0][0];
                                    foreach($ro_c as $r){
                                        $xc += $r[0];
                                    }
                                    $xxx = $rowa[2] - $xc;
                                    if( $xxx > 0){
                                        ...
                                        $html .= "<td class='campos text-right'>".number_format($valorAprox, 2, ',', '.')."</td>";
                                        $html .= "<td class='campos text-right'>".number_format($iva, 2, ',', '.')."</td>";
                                        $html .= "<td class='campos text-right'>".number_format($rowa[8], 2, ',', '.')."</td>";
                                        $vtt = ($rowa[3] + $rowa[6]+$rowa[8]) * $xxx;
            ...
            <div class="col-lg-10 col-md-10 col-sm-10 text-right">
                ...
                <label for="" class="control-label col-lg-1 col-md-2 col-sm-1 text-right values" title="Total de Iva"><?php echo "$".number_format($sumIva,2,',','.') ?></label>
                <label for="" class="control-label col-lg-1 col-md-2 col-sm-1 text-right values" title="Total de ImpoConsumo"><?php echo "$".number_format($sumImpo,2,',','.') ?></label>
    ...
    <div class="modal fade" id="exportarEntrada" role="dialog" align="center" >
        <div class="modal-dialog">
            <div class="modal-content" style="width: 500px;">
                <div id="forma-modal" class="modal-header">
                    <h4 class="modal-title" style="font-size: 24; padding: 3px;">Informe Entrada Almacén</h4>
                </div>
                <div class="modal-body" style="margin-top: 8px">
    ...
    <script>
        ...
        function modify_data() {
            var form_data = {
                ...
                sltCentroCosto  : $("#sltCentroCosto").val(),
                sltProyecto     : $("#sltProyecto").val(),
                sltDependencia  : $("#sltDependencia").val(), 
                responsable     : $("#sltResponsable").val(),
                sltRubroP       : $("sltRubroP").val(),
                tercero         : $("#sltTercero").val(),
                sltUPE          : $("#sltUPE").val(),
                txtPlazoE       : $("#txtPlazoE").val(),
                sltLE           : $("#sltLE").val(),
                txtIva          : $("#txtIva").val(),
                sltDocSoporte   : $("#sltDocSoporte").val(),
                txtNumDocS      : $("#txtNumDocS").val(),
                sltFuente       : $("#sltFuente").val(),
        ...
        $("#txtValorIva").change(function(){ 
            let iva = $("#txtValorIva").val();
            if(iva<100){ 
                $("#txtValorImpo").val(0);
                calcularIva('txtValorIva', iva);
            }
        })
        $("#txtValorImpo").change(function(){ 
            let impo = $("#txtValorImpo").val();
            if(impo<100){
                $("#txtValorIva").val(0);
                calcularIva('txtValorImpo', impo);
            }
        })
        function calcularIva(campo, valor){
            let cantidad    = $("#txtCantidad").val();
            let val         = $("#txtValor").val();
            if(val !=''){
                if(cantidad == ''){
                    $("#txtCantidad").val(1);
                    cantidad = 1;
                }
                let vi          = (val * valor/100); 
                vi = parseFloat(vi).toFixed(2);
                $("#"+campo).val(vi); 
                total    = (parseFloat(val) + parseFloat(vi)) * cantidad;
        ...
        $("#txtValor").change(function(){
            ...
            valor    = parseFloat($("#txtValor").val());
            iva      = parseFloat(<?php echo $porcIva; ?>);
            if($("#txtValorIva").val()>0 || $("#txtValorImpo").val()>0){
                impo        = $("#txtValorImpo").val();
                totalIva    = $("#txtValorIva").val();
            } else {
                if(<?php echo $dc_[0] ?> != ' '){
                    totalIva = redondeo((valor * iva)/100, <?php echo $dc_[0] ?>);
                }else{
            ...
            total    = (parseFloat(valor) + parseFloat(totalIva)+ parseFloat(impo)) * cantidad;            
            $("#txtValorTotal").val(total);
            $("#txtAjuste").val(0);
        });

        $("#txtAjuste").change(function(){
            var ajuste  = $("#txtAjuste").val();
            let iva     = $("#txtValorIva").val();
            let impo    = $("#txtValorImpo").val();
            ...
            total    = ((parseFloat(valor) + parseFloat(iva)+ parseFloat(impo)) * cantidad)+parseFloat(ajuste);
        ...
        function exportarEntrada(){
            if($("#exportarE").val()==1){ 
                window.open('informes_almacen/inf_entrada_almacen.php?t=1&mov=<?php echo md5($id)?>');
            } else {
                window.open('informes_almacen/inf_entrada_almacen.php?t=2&mov=<?php echo md5($id)?>');
            }
...
$(document).ready(function(){
    ...
    // Verificar que no tenga comprobante cnt
    let id = $("#txtIdMov").val();
    let form_data = {action: 7, id:id};
    $.ajax({
        type: "POST",
        url: "jsonPptal/gf_almacenJson.php",
        data: form_data,
        success: function (response)
        {
            ...
            if (response != 0) {
                let comprobante = response.trim();
                $("#id_comprobante_cnt").val(comprobante);
                $("#btnModificar").attr('disabled',true);
                $("#btnOrden").attr('disabled',true);
                $("#btnDocumento").attr('disabled',true);
                $("#btnGuardarDetalle").attr('disabled',true);
                $(".eliminar").attr('disabled',true);
                $("#btnGuardarP").css("display", "none");
                $("#btnVerC").attr('disabled',false);
                $("#btnCrearC").attr('disabled',true);
                $("#btnSubir").attr('disabled',true);                
                $(".modificar").attr('disabled',true);
                $(".modificar").css("display", "none");
                $(".eliminar").css("display", "none");
            }  else {
                $("#btnVerC").attr('disabled',true);
                $("#btnCrearC").attr('disabled',false);
...
<div class="modal fade" id="mdlSubir" role="dialog" align="center" >
    <div class="modal-dialog">
        <div class="modal-content">
            <div id="forma-modal" class="modal-header">
                <h4 class="modal-title" style="font-size: 24px; padding: 3px;">Información</h4>
            ...
            <div class="modal-body" style="margin-top: 8px">
                <form id="formArchivo" name="formArchivo">
                    <div class="form-group">
                        <p align="center" style="margin-bottom: 25px; margin-top: 25px; margin-left: 30px; font-size: 80%">Archivo .xsl - .xlsx <a href="documentos/formatos/Formato_Entrada.xlsx" target="_blank"><i class="fa fa-file-excel-o"></i></a></p>
                        <input type="hidden" name="idMov" id="idMov" value="<?= $id; ?>">
                        <label class="col-lg-2 col-md-2 col-sm-2 control-label" for="sltArchivo">Archivo:</label>
            ...
            <div id="forma-modal" class="modal-footer">
                <button type="button" id="btnSubirA" class="btn" style="color: #000; margin-top: 2px" data-dismiss="modal">Subir</button>
                <button type="button" id="btnCancelarS" class="btn" style="color: #000; margin-top: 2px" data-dismiss="modal">Cancelar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
//FUNCION SUBIR DATOS POR ARCHIVO PLANO
$("#btnSubir").click(function(){
    $("#mdlSubir").modal("show");
   
})
$("#btnSubirA").click(function(){
    jsShowWindowLoad('Validando Información...');
    var formData = new FormData($("#formArchivo")[0]);  
    $.ajax({
        type: 'POST',
        url: "jsonPptal/gf_almacenJson.php?action=8",
        data:formData,
        contentType: false,
        processData: false,
        success: function(response)
        { 
            ...
            if(rta>0){
                ...
                $.ajax({
                    type: 'POST',
                    url: "jsonPptal/gf_almacenJson.php?action=9",
                    data:formData,
                    contentType: false,
                    processData: false,
                    success: function(response)
                    { 
                        ...
                        if(response==0){
                            $("#txtmsjmdl").html('No Se Ha Podido Cargar la Información');
                            $("#mdlMensaje").modal("show");
                        } else {
                            $("#txtmsjmdl").html(response+' Registros Subidos Correctamente');
                            $("#mdlMensaje").modal("show");
                        }
                        $("#btnMsj").click(function(){
                            document.location.reload();
...
//INTERFAZ 
$("#btnCrearC").click(function(){
    jsShowWindowLoad('Validando Datos');
    let id =$("#txtIdMov").val(); 
    let form_data = {action: 10,idMov:id };
    $.ajax({
        type: "POST",
        url: "jsonPptal/gf_almacenJson.php",
        data: form_data,
        ...
        {
            jsRemoveWindowLoad();
            console.log(response+'Validar Cierre');
            let resultado = JSON.parse(response);
            let rta = resultado["rta"];
            let txt = resultado["msj"];
            if(rta>0){
                ...
                jsShowWindowLoad('Guardando Información');
                let form_data = {action: 11,idMov:id };
                $.ajax({
                    type: "POST",
                    url: "jsonPptal/gf_almacenJson.php",
                    data: form_data,
                    success: function (response)
                    {
                        jsRemoveWindowLoad();
                        if(response==0){
                            $("#txtmsjmdl").html('No Se Ha Podido Guardar La Información');
                            $("#mdlMensaje").modal("show");
                        } else {
                            $("#txtmsjmdl").html(response+' Registros Subidos Correctamente');
                            $("#mdlMensaje").modal("show");
...
$("#btnVerC").click(function(){
    let comprobante = $("#id_comprobante_cnt").val();
    window.open('registrar_GF_COMPROBANTE_CONTABLE.php?id='+md5(comprobante));
})
</script>