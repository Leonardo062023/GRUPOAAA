...
list($tipo, $numero, $fecha, $dependencia, $responsable, $descripcion) = array("", "", "", "", "", "");
$cierre = 0;
if(!empty($_GET['reintegro'])){
    $data = $rig->obtnerReintegro($_GET['reintegro']);
    list($fecha, $numero, $dependencia, $responsable) = array($data[1], $data[2], "$data[6] ".ucwords(mb_strtolower($data[7])), ucwords(mb_strtolower($data[9]))." ".$data[9], $data[10]);
    $tipo = "$data[4] ".ucwords(mb_strtolower($data[5]));
    $cierre = cierreFecha($data[11]);
}
?>
                ...
                <div class="client-form contenedorForma">
                    <form name="form" id="form" class="form-horizontal" method="GET"  enctype="multipart/form-data" action="registrar_RF_REINTEGRO.php">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-2 col-md-2 col-lg-2">
                                <select name="sltDepOr" id="sltDepOr" class="form-control select2" required="" onchange="obtnerResponsableOrg($(this).val())">
                                    ...
                                    if(empty($depOr)){
                                        ...
                                        $sql_dep = "SELECT dep.id_unico, UPPER(dep.sigla), dep.nombre FROM gf_dependencia dep WHERE md5(dep.id_unico) != '$depOr' AND dep.tipodependencia = 6 AND dep.compania = $compania AND (movimiento =0 OR activa = 0) ";
                            ...
                            <label for="sltDepDe" class="control-label col-sm-1 col-md-1 col-lg-1"><strong class="obligado">*</strong>Dependencia Destino:</label>
                            <div class="col-sm-2 col-md-2 col-lg-2">
                                <select name="sltDepDe" id="sltDepDe" class="form-control select2" required="" onchange="obnterResponsableDes($(this).val())">
                                    ...
                                    if(!empty($dependencia)){
                                        ...
                                        if(empty($depDe)){
                                            ...
                                            $sql  = "SELECT dep.id_unico, UPPER(dep.sigla), dep.nombre FROM gf_dependencia dep WHERE md5(dep.id_unico) != '$depDe' AND dep.tipodependencia IN(1,2) AND dep.compania = $compania AND (movimiento =0 OR activa = 0) ";
                        ...
                        if(!empty($_GET['reintegro'])){
                            ...
                            foreach($datD as $row){
                                $vp = $con->Listar("SELECT DISTINCT ef.nombre as plan, pre.valor as serie ,pr.descripcion 
                                        ...
                                        WHERE  mpr.detallemovimiento = ".$row[0]." AND pre.fichainventario != 6");
                                ...
                                #*¨Buscar Afectado 
                                $af = $con->Listar("SELECT * FROM gf_detalle_movimiento WHERE detalleasociado = ".$row[0]);
                                if(empty($af[0][0])){
                                    if($cierre == 0){
                                        $html .= "<a href=\"javascript:void(0)\" onclick=\"eliminarD($row[0],$row[5])\"><span class=\"glyphicon glyphicon-trash\"></span></a>";
                                    }
                                }
                                 $html .= "</td>";
                                $html .= "\n\t</tr>";
