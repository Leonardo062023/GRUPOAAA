...
if ($action == 1) {
    ...
    if (isset($_GET["id_asociadoJur"])) {
        $id_asociadoJur = base64_decode(($_REQUEST["id_asociadoJur"]));
        switch ($_SESSION['conexion']) {
         ...
         WHERE t.Id_Unico = '$id_asociadoJur'");
                break;
            case 2:
                $row  = $con->Listar("SELECT t.Id_Unico, 
            t.RazonSocial, 
            ...
            ci.departamento , 
            rl.id_unico, 
           ' '|| rl.NombreUno||rl.NombreDos||rl.ApellidoUno||rl.ApellidoDos,
            rl.NumeroIdentificacion, 
            d.nombre, ci.nombre ,
            ct.id_unico, 
            ' '||ct.NombreUno||ct.NombreDos||ct.ApellidoUno||ct.ApellidoDos,
            ct.NumeroIdentificacion 
         FROM gf_tercero t 
         LEFT JOIN gf_tipo_identificacion ti ON t.TipoIdentificacion = ti.Id_Unico
         LEFT JOIN gf_sucursal s ON t.Sucursal = s.Id_Unico
...
ORDER BY t.NombreUno ASC");
  ...
  ORDER BY Nombre ASC");
    ...
    #****** Ciudad  *********#
    if (empty($row[0][23]) && empty($row[0][22])) {
        $de = 0;
        $cd = 0;
    } else {
        $de = $row[0][23];
...
if ($action == 2) {
    $_SESSION['perfil'] = "J"; //Jurídica.
    $_SESSION['url'] = "registrar_GF_ASOCIADO_JURIDICA.php";

    #****** Tipo Identificación *********#
    $tipoIden  = $con->Listar("SELECT DISTINCT Id_Unico, Nombre 
      FROM gf_tipo_identificacion
      ORDER BY Nombre ASC");
    ...
    <div class="container-fluid text-center">
        <div class="content row">
            ...
            <div class="col-sm-7 text-left" style="margin-left: -16px;margin-top: -20px;">
                ...
                ?>
                <?php if ($action == 2) { ?>
                    <h2 align="center" class="tituloform">Registrar Asociado Jurídica</h2>
                <?php }
                ...
                <div class="client-form contenedorForma">
                    <form name="form" id="form" class="form-horizontal" method="POST" enctype="multipart/form-data" action="">
                        ...
                        <input  type="hidden"  name="perfil" value=8>
                        <input  type="hidden"  name="id" value="<?php echo $id_asociadoJur; ?>">
                        <div class="form-group form-inline " style="">
                            ...
                            <div class="form-group form-inline col-sm-3" style="">
                                <?php if ($action == 1) { ?>
                                    <input type="text" name="noIdent" id="noIdent" class="form-control col-sm-5" maxlength="20" title="Ingrese el número de identificación" onkeypress="return txtValida(event, 'num')" placeholder="Número" style="width:95px" style="height: 30px" required onblur="CalcularDv();    return existente()" value="<?php echo $row[0][2]; ?>" />
                                <?php }
                            ...
                            <?php if ($action == 1) { ?>
                                <select name="sucursal" id="sucursal" class="select2_single form-control" title="Seleccione la sucursal">
                                    ...
                                    for ($i = 0; $i < count($sucursal); $i++) {
                                        echo '<option value="' . $rowS[$i][0] . '">' . ucwords((mb_strtolower($rowS[$i][1]))) . '</option>';
                                    }
                                    ?>
                                </select>
                            <?php }
                        ...
                        <div class="form-group" style="margin-top: -10px; ">
                            ...
                            <?php if ($action == 1) { ?>
                                <input type="text" name="razoSoci" id="razoSoci" class="form-control" maxlength="500" title="Ingrese la razón social" value="<?php echo ($row[0][1]); ?>" onkeypress="return txtValida(event, 'num_car')" onkeyup="javascript:this.value = this.value.toUpperCase();" placeholder="Razón Social" required>
                            <?php }
                            ...
                            <?php if ($action == 1) { ?>
                                <select name="tipoEmp" id="tipoEmp" class="select2_single form-control" title="Ingrese el tipo de empresa">
                                    ...
                                    for ($i = 0; $i < count($tipoEmp); $i++) {
                                        echo '<option value="' . $tipoEmp[0][0] . '">' . ucwords((mb_strtolower($tipoEmp[0][1]))) . '</option>';
                                    }
                                    ?>
                                </select>
                            <?php }
                        ...
                        <div class="form-group" style="margin-top: -10px; ">
                            ...
                            <?php if ($action == 1) { ?>
                                <select name="repreLegal" id="repreLegal" class="select2_single form-control" title="Ingrese el representante legal">
                                    <?php
                                    if (!empty($row[0][8])) {
                                        echo '<option value="' . $row[0][24] . '">' . ucwords(mb_strtolower($row[0][25])) . ' - ' . $row[0][26] . '</option>';
                                        echo '<option value=""> - </option>';
                                    } else {
                                        echo '<option value=""> - </option>';
                                    }
                            ...
                            <div class="form-group form-inline col-sm-3" style="margin-left:-10px">
                                <?php if ($action == 1) { ?>
                                    <select name="depto" id="depto" class="select2_single form-control col-sm-5" style="height: 20%;width:170px" title="Seleccione Departamento" required>
                                        <?php
                                        if (!empty($row[0][23])) {
                                            echo '<option value="' . $row[0][23] . '">' . ucwords(mb_strtolower($row[0][27])) . '</option>';
                                        } else {
                                            echo '<option value=""> - </option>';
                                        } ?>
                            ...
                            <div class="form-group form-inline col-sm-3" style="">
                                <?php if ($action == 1) { ?>
                                    <select name="ciudad" style="height: 24%;width:100px" id="ciudad" class="select2_single form-control" title="Seleccione Ciudad" required>
                                        <?php
                                        if (!empty($row[0][22])) {
                                            echo '<option value="' . $row[0][22] . '">' . ucwords(mb_strtolower($row[0][28])) . '</option>';
                                        } else {
                                            echo '<option value=""> - </option>';
                                        } ?>
                            ...
                            <?php if ($action == 1) { ?>
                                <select name="contacto" id="contacto" class="select2_single form-control" title="Ingrese el contacto">
                                    <?php if (empty($row[0][13])) {
                                        ...
                                        echo '<option value="' . $row[0][29] . '">' . ucwords(mb_strtolower($row[0][30])) . ' - ' . $row[0][31] . '</option>';
                                        echo '<option value=""> - </option>';
                                    }
                                    for ($i = 0; $i < count($contacto); $i++) {
                                        echo '<option value="' . $contacto[$i][0] . '">' . $contacto[$i][1] . ' ' . $contacto[$i][2] . ' ' . $contacto[$i][3] . ' ' . $contact[$i][4] . '(' . $contacto[$i][5] . ' - ' . $contacto[$i][6] . ')' . '</option>';
                                    }
                                    ?>
                                </select>
                            <?php }
                            ...
                            <?php if ($action == 1) { ?>
                                <select name="zona" id="zona" class="select2_single form-control" title="Ingrese la zona">
                                    ...
                                    for ($i = 0; $i < count($zona); $i++) {
                                        echo '<option value="' . $zona[$i][0] . '">' . ucwords((mb_strtolower($zona[$i][1]))) . '</option>';
                                    }    ?>
                                </select>
                            <?php }
                        ...
                        <div class="form-group" style="margin-top: -10px;">
                            ...
                            <?php if ($action == 1) { ?>
                                <input type="email" name="correo" id="correo" value="<?php echo $row[0][21] ?>" class="form-control" maxlength="500" title="Ingrese Correo Electrónico" placeholder="Corrreo Electrónico">
                            <?php }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response == 1) {
                                ...
                                document.location = 'listar_GF_ASOCIADO_JURIDICA.php';
