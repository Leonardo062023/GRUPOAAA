...
$row = $con->Listar("SELECT distinct id_unico, codigo_interno FROM gp_facturacion_servicios_inicial  where total_aseo is null ORDER BY id_unico ");
for ($i=0; $i <count($row) ; $i++) {  
	...
	$rowj = $con->Listar("SELECT DISTINCT cuenta_contrato, mora, 
		...
		from gp_aseo a WHERE a.periodo = 15  
		and cuenta_contrato like '%".$codigo."'
		order by id_unico ASC ");
	$ij =0;
	 $sql = "update gp_facturacion_servicios_inicial  
		set 
		mora 					='".$rowj[$ij][1]."', 
		...
		echo $row[$i][0].' Cuenta_CONTRATO:' .$codigo.'- R:'.$resultadoP.'<br/>';
...
/*
	...
	for ($ij=0; $ij < count($rowj) ; $ij++) { 
		...
		$codigo_interno = $ud[0][1];
		 ...
		 $sql = "update gp_facturacion_servicios 
		set 
		mora 					='".$rowj[$ij][1]."', 
		tipo_cliente			='".$rowj[$ij][2]."', 
		cft 					='".$rowj[$ij][3]."',
