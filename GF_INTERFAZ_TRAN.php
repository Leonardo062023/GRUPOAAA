...
#********************Consulta el parametro de dígitos Código********************
 $cona= "SELECT id_unico, nombre, valor   
                FROM gs_parametros_basicos 
                WHERE nombre='Dígitos Interfaz Inventario'";
  $resultC = $mysqli->query($cona);
  $rowCo = mysqli_fetch_row($resultC);
  $dig =$rowCo[2];;
...
$ttmov2 = $conex->Listar("SELECT *  
                FROM gf_tipo_movimiento WHERE compania = $compania 
                ORDER BY sigla ASC");
#********************Cuenta********************#
    ...
    if(isset($_GET['grupo'])){
        if($_GET['interfaz']==1){
            $cons = $conex->Listar("SELECT ca.id_unico, 
                ...
                CONCAT_WS(' - ',civ.codi_cuenta,LOWER(civ.nombre)), 
                CONCAT_WS(' - ',cim.codi_cuenta,LOWER(cim.nombre)), 
                civ.id_unico, cim.id_unico 
            ...
            LEFT JOIN 
                gf_cuenta civ ON ca.cuenta_iva = civ.id_unico 
            LEFT JOIN 
                gf_cuenta cim ON ca.cuenta_impo = cim.id_unico 
            WHERE 
            ...
            $cons = $conex->Listar("SELECT ca.id_unico, 
                ...
                cb.id_unico, cd.id_unico, cc.id_unico  , 0, 0, 0,0 
            FROM gf_configuracion_almacen ca 
            LEFT JOIN 
                gf_cuenta cd ON ca.cuenta_debito = cd.id_unico 
            LEFT JOIN 
    ...
    <div class="container-fluid text-center"  >
        <div class="row content">
            ...
            <div class="col-sm-10" style="margin-left: -10px;margin-top: 5px" >
                                ...
                                <?php if(isset($_GET['interfaz'])){
                                        if(isset($_GET['grupo'])){ ?>
                                            ...
                                            <?php if($_GET['interfaz']==1) { ?>
                                                ...
                                                <div class="form-group form-inline col-sm-12" style="margin-top: -15px; margin-left: 20px;"> 
                                                    <div class="col-sm-4" align="left" style="margin-left:-20px">  
                                                        <label for="ctaIva" class="control-label" ><strong style="color:#03C1FB;"></strong>Cuenta Iva:</label><br>
                                                        <select name="ctaIva" id="ctaIva" class="select2_single form-control input-sm" title="Seleccione Cuenta Iva" style="width:250px; ">
                                                    ...
                                                    <div class="col-sm-4" align="left" style="margin-left:-20px">  
                                                        <label for="ctaImpo" class="control-label" ><strong style="color:#03C1FB;"></strong>Cuenta Impoconsumo:</label><br>
                                                        <select name="ctaImpo" id="ctaImpo" class="select2_single form-control input-sm" title="Seleccione Cuenta Impoconsumo" style="width:250px; ">
                                                            <option value="">Cuenta Impoconsumo</option>
                                                            <?php 
                                                            for($i=0;$i<count($ccuenta); $i++){
                                                                echo '<option value="'.$ccuenta[$i][0].'">'.$ccuenta[$i][1].' - '.ucwords(mb_strtolower($ccuenta[$i][2])). '</option>';
                                                            }
                                                    ...
                                                    <div class="col-sm-4" align="right"  style="margin-top: 35px;margin-left: -75px;">
                                                        <button type="button" onclick="guardar1()"  id="btnGuardar" class="btn btn-primary sombra" style="background: #00548F; color: #fff; border-color: #1075C1;" title="Guardar"><li class="glyphicon glyphicon-floppy-disk"></li></button> <!--Guardar-->
                                    ...
                                    if($_GET['interfaz']==1){ ?>
                                        <thead>
                                            <tr>
                                                ...
                                                <td><strong>Cuenta Iva</strong></td>
                                                <td><strong>Cuenta Impoconsumo</strong></td>
                                            </tr>
                                            <tr>
                                                ...
                                                <th width="30%">Cuenta Iva</th>
                                                <th width="30%">Cuenta Impoconsumo</th>
                                            </tr>
                                        </thead> 
                                        ...
                                        <?php for($i=0; $i<count($cons);$i++){?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    if(empty($cons[$i][9])) { $civ = 0;}else{$civ = $cons[$i][9];} 
                                                    if(empty($cons[$i][10])) { $cim = 0;}else{$cim = $cons[$i][10];} ?>
                                                    <a href="javascript:modificar(<?php echo $cons[$i][0].','.$cb.','.$cons[$i][5].','.$cons[$i][6].','.$civ.','.$cim?>);">
                                                ...
                                                if($_GET['interfaz']==1){
                                                    echo "<td>".ucwords($cons[$i][7])."</td>";
                                                    echo "<td>".ucwords($cons[$i][8])."</td>";
             ...
             <script>
                function modificar(id, bt, cd, cc, ci, cim){
                    $("#idmod").val(id);
                    $("#tipcmod").val(bt);
                    $("#ctadmod").val(cd);
                    $("#ctacmod").val(cc);
                    ...
                    <div class="modal-content client-form1">
                        ...
                        <div class="modal-body ">
                            <form  name="form" method="POST" action="javascript:modificarItem()">
                                    ...
                                    <?php if($_GET['interfaz']==1){ ?>
                                        ...
                                        <select style="display:inline-block; width:250px; margin-bottom:15px; height:40px" name="tipcmod" id="tipcmod" class="select2_single form-control" title="Seleccione Tipo Movimiento" required>
                                                ...
                                                for($i=0;$i<count($ttmov2); $i++){
                                                    echo '<option value="'.$ttmov2[$i][0].'">'.mb_strtoupper( $ttmov2[$i][1]).' - '.ucwords(mb_strtolower($ttmov2[$i][2])). '</option>';
                                ...
                                <?php if($_GET['interfaz']==1){ ?>
                                <div class="form-group" style="margin-top: 13px;">
                                    <label style="display:inline-block; width:140px"><strong style="color:#03C1FB;"></strong>Cuenta IVA:</label>
                                    <select style="display:inline-block; width:250px; margin-bottom:15px; height:40px" name="ctaIvmod" id="ctaIvmod" class="select2_single form-control" title="Seleccione Cuenta IVA" >
                                            <option value="">Cuenta IVA </option>
                                            <?php 
                                            for($i=0;$i<count($ccuenta); $i++){
                                                echo '<option value="'.$ccuenta[$i][0].'">'.$ccuenta[$i][1].' - '.ucwords(mb_strtolower($ccuenta[$i][2])). '</option>';
                                ...
                                <div class="form-group" style="margin-top: 13px;">
                                    <label style="display:inline-block; width:140px"><strong style="color:#03C1FB;"></strong>Cuenta Impoconsumo:</label>
                                    <select style="display:inline-block; width:250px; margin-bottom:15px; height:40px" name="ctaImmod" id="ctaImmod" class="select2_single form-control" title="Seleccione Cuenta Crédito" >
                                            <option value="">Cuenta Impoconsumo </option>
                                            <?php 
                                            for($i=0;$i<count($ccuenta); $i++){
                                                echo '<option value="'.$ccuenta[$i][0].'">'.$ccuenta[$i][1].' - '.ucwords(mb_strtolower($ccuenta[$i][2])). '</option>';
            ...
            <script>
                function modificarItem(){
                    ...
                    var ctaIv = $("#ctaIvmod").val();
                    var ctaIm = $("#ctaImmod").val();
                    if(ctad=='' || ctac==''){
                        $("#mensaje").html('Datos Incompletos');
                        $("#myModalError").modal("show");
                    } else {
