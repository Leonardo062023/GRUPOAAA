...
for ($i=0; $i < count($rowe); $i++) { 
    ...
    $ld = "DELETE  n.* FROM gn_novedad n 
        ...
        WHERE n.periodo = $periodo AND n.empleado = $empleado AND c.codigo in ('097','140', '144', '002', 'L007', 'L006', '150') ";
    $resultado1 = $mysqli->query($ld);

    $sueldo  = sueldo($id_empleado);
    #SUELDO 
    $id_concepto    = id_concepto('002');
    guardarNovedad($sueldo, $id_empleado, $periodo, $id_concepto);

    #*factor
    guardarActual($id_empleado,'L007', $periodo );

    #*factor días
    guardarActual($id_empleado,'L006', $periodo );
     
    #Valor Prima
    
    $Vprima         = ROUND($sueldo * 3.5);
    $id_concepto    = id_concepto('150');
    guardarNovedad($Vprima, $id_empleado, $periodo, $id_concepto);
    ...
    $dv = $con->Listar("SELECT SUM(n.valor) FROM gn_novedad n 
        ...
        AND c.clase = 1 and c.unidadmedida = 1 AND c.codigo not in ('002')");
...
$(document).ready(function() {
    ...
    if(response==0){
        ...
        $("#mdlInfo").modal('show'); 
        $("#ver1").click(function(){
            document.location ='../informes_nomina/generar_INF_SABANA_PRIMAA.php?t=1&e=<?=$empleador?>&p=<?=$periodo?>';
        })
