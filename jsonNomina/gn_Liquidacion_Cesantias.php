...
for ($i=0; $i < count($rowe); $i++) {
    ...
    #188
    #Buscar cesantias r del año pasado 
    $fechaanI = $annoa.'-12-01';
    $fechaanF = $annoa.'-12-31';
    $cran = $con->Listar("SELECT n.valor FROM gn_novedad n 
        LEFT JOIN gn_concepto c ON c.id_unico = n.concepto 
        LEFT JOIN gn_periodo p ON n.periodo = p.id_unico 
        WHERE p.fechainicio BETWEEN '$fechaanI' AND '$fechaanF'
        AND n.empleado = $empleado AND c.codigo = '189'");

    if(empty($cran[0][0])){
        $vl188 = $vcenr;
    } else {
        $vl188 = $vcenr - $cran[0][0];
    }
    if($vl188!=0){
        $id_cn188     = id_concepto('188');
        guardarNovedad($vl188, $empleado, $periodo, $id_cn188);
    }
...
if($empleador ==2){
    $rowe = $con->Listar("SELECT DISTINCT e.id_unico, 
        ...
        LEFT JOIN gn_grupo_gestion gg oN e.grupogestion = gg.id_unico         
        LEFT JOIN gn_categoria_riesgos cr ON e.tipo_riesgo = cr.id_unico 
        WHERE e.id_unico != 2 AND e.cesantias = 1
        AND (((SELECT vr.estado FROM gn_vinculacion_retiro vr where vr.empleado = e.id_unico AND vr.fechaacto<= '$fechaFin'  ORDER BY vr.fechaacto DESC LIMIT 1)=1) or  ((SELECT vr.estado FROM gn_vinculacion_retiro vr where vr.empleado = e.id_unico AND vr.fechaacto BETWEEN '$fechaInicio' AND '$fechaFin' ORDER BY vr.fechaacto DESC LIMIT 1)=2 ))
        ORDER BY e.id_unico"); 
} else {
    $rowe = $con->Listar("SELECT DISTINCT e.id_unico, 
        ...
        LEFT JOIN gn_grupo_gestion gg oN e.grupogestion = gg.id_unico         
        LEFT JOIN gn_categoria_riesgos cr ON e.tipo_riesgo = cr.id_unico
        WHERE e.id_unico = $empleador  AND e.cesantias = 1
        AND (((SELECT vr.estado FROM gn_vinculacion_retiro vr where vr.empleado = e.id_unico AND vr.fechaacto<= '$fechaFin'  ORDER BY vr.fechaacto DESC LIMIT 1)=1) or  ((SELECT vr.estado FROM gn_vinculacion_retiro vr where vr.empleado = e.id_unico AND vr.fechaacto BETWEEN '$fechaInicio' AND '$fechaFin' ORDER BY vr.fechaacto DESC LIMIT 1)=2 )) 
        ORDER BY e.id_unico"); 
}
...
for ($i=0; $i < count($rowe); $i++) {
    ...
    #Ultimo periodo cesantias
    $fechar = new DateTime($fechaSalida);
    $diasc  = diasPendientes2(11, $empleado, $fechaIngreso, $fechar, $fechaSalida, $periodo);
    $id_dcn =  id_concepto('1030');
    guardarNovedad($diasc, $empleado, $periodo, $id_dcn);    
    //var_dump($diasc);
...
?>
    ...
    $(document).ready(function() {
        ...
        if(response>=1){
            ...
            $("#mdlInfo").modal('show'); 
            document.location ='../liquidar_GN_CESANTIAS.php';
        }
    })
    </script>
