...
$pdf->Multicell(155, 5, utf8_decode($razonSoc),  0, 'L');
...
$pdf->CellFitScale(45, $alt, utf8_decode('Código'), 1, 0, 'C');
$pdf->Cell(65, $alt, utf8_decode('Rubro Fuente'), 1, 0, 'C');
...
while ($rowG = mysqli_fetch_array($detalle)) {
    ...
    $x1 = $pdf->GetX();
    $pdf->Cell(95, 5, ' ', 0, 0, 'L');
    $pdf->MultiCell(65, 5, utf8_decode($nombreCuenta), 1, 'J');
    $y2 = $pdf->GetY();
    $h = $y2 - $y1;
    $px = $x1 + 65;
...
while ($rowMF = mysqli_fetch_array($movimientoFinanciero)) {
    ...
    if (strlen($nombCuen) > 35 || strlen($centroCost) > 35) {
        $altY = $pdf->GetY();
        if ($altY > 310) {
            $pdf->AddPage();
            $paginactual = $pdf->PageNo();
...
while ($rowTipComp = mysqli_fetch_array($tipComp))
{
     if(!empty($rowTipComp[5])){
            if($fechaComprobante <=$rowTipComp[5] && $fechaComprobante >=$rowTipComp[4]){
