<?php
    ...
    class PDF extends FPDF
    {
        ...
        { 
            ...
            $this->SetFont('Arial','',10);
            $this->Cell(330,10,utf8_decode('RESUMEN DE DECLARACION DE RETEICA'),0,0,'C');
            $this->Ln(10);
    ...
    if(empty($fecF) ){
        ...
        if($val == 1){
            $sql2 = "SELECT DISTINCT dd.concepto,  cc.descripcion, cc.nom_inf FROM gc_declaracion d
                    LEFT JOIN 	gc_detalle_declaracion dd ON dd.declaracion = d.id_unico
                    LEFT JOIN gc_concepto_comercial cc ON dd.concepto = cc.id_unico
                    WHERE d.fecha BETWEEN '$fechaI' AND '$fechaF' AND d.clase = 2 AND dd.tipo_det = 1 ORDER BY cc.codigo ASC";
            
            $sql3 = "SELECT COUNT(DISTINCT dd.concepto) FROM gc_declaracion d
                    LEFT JOIN gc_detalle_declaracion dd ON dd.declaracion = d.id_unico
                    LEFT JOIN gc_concepto_comercial cc ON dd.concepto =cc.id_unico
                    WHERE d.fecha BETWEEN '$fechaI' AND '$fechaF' AND d.clase = 2 AND dd.tipo_det = 1 ";       
            ...
            $sql2 = "SELECT DISTINCT dd.concepto,  cc.descripcion, cc.nom_inf FROM gc_declaracion d
                    LEFT JOIN 	gc_detalle_declaracion dd ON dd.declaracion = d.id_unico
                    LEFT JOIN gc_concepto_comercial cc ON dd.concepto = cc.id_unico
                    WHERE d.fecha BETWEEN '$fechaI' AND '$fechaF' AND d.clase = 2 AND dd.tipo_det = 1  AND d.id_unico NOT IN(SELECT declaracion FROM gc_recaudo_comercial) ORDER BY cc.codigo ASC";
    ...
    $pdf->Cell(10,5, utf8_decode('Cod. D'),0,0,'C');
    ...
    while($CR1 = mysqli_fetch_row($CR)){
        ...
        $pdf->Cellfitscale(20,8, utf8_decode($fechaD),1,0,'C');  
         $pdf->Cellfitscale(10,8, utf8_decode($CR1[5]),1,0,'C');
    ...
    $pdf->Cell(100,8, utf8_decode('Total:'),1,0,'C');
    ...
    while($Tcon3 = mysqli_fetch_row($con3)){
        ...
        if(!empty($fecF)){
            ...
            if($val == 1){
                ...
                echo $sql5 = "SELECT SUM(dd.valor), dd.concepto FROM gc_detalle_declaracion dd 
                        LEFT JOIN gc_declaracion d ON dd.declaracion = d.id_unico 
                        LEFT JOIN gc_recaudo_comercial rc ON rc.declaracion = d.id_unico  
                        WHERE dd.concepto = '$Tcon3[0]' AND d.fecha BETWEEN '$fechaI' AND '$fechaF' AND d.clase = 2 AND dd.tipo_det = 1";
            ...
            if($val == 1){
                echo $sql5 = "SELECT SUM(dd.valor)
                    FROM gc_detalle_declaracion dd
                    LEFT JOIN  gc_declaracion d ON dd.declaracion = d.id_unico
                    WHERE dd.concepto = '$Tcon3[0]' AND d.clase = 2 AND dd.tipo_det = 1";
                 echo "<br/>";
            }elseif($val == 2){
                echo $sql5 = "SELECT SUM(dd.valor), dd.concepto 
                        FROM gc_detalle_declaracion dd 
                        LEFT JOIN gc_declaracion d ON dd.declaracion = d.id_unico 
                        LEFT JOIN gc_recaudo_comercial rc ON rc.declaracion = d.id_unico  
                        WHERE dd.concepto = '$Tcon3[0]' AND rc.fecha >= '$fechaI' AND d.clase = 2 AND dd.tipo_det = 1";
                echo "<br/>";
            }else{
            ...
            $sql5 = "SELECT SUM(dd.valor)
                    FROM gc_detalle_declaracion dd
                    LEFT JOIN gc_declaracion d ON dd.declaracion = d.id_unico LEFT JOIN gc_recaudo_comercial rc ON rc.declaracion = d.id_unico
                    WHERE dd.concepto = '$Tcon3[0]' AND d.clase = 2  AND dd.tipo_det = 1 AND  d.fecha >= '$fechaI' AND dd.declaracion NOT IN (SELECT rc.declaracion FROM gc_recaudo_comercial rc) ";
