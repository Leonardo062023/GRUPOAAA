...
if (!empty($_REQUEST['id'])) {
    ...
    $adicion = base64_decode($_REQUEST['id']);

    switch ($conexion) {
        case 1:
            $modif = $con->Listar("SELECT com.id_unico,
            com.numero,
            ...
            WHERE
                com.id_unico = $adicion");
            break;
        case 2:
            $modif = $con->Listar("SELECT com.id_unico, com.numero, TO_CHAR(com.fecha, 'DD-MM-YYYY'), 
            TO_CHAR(com.fechavencimiento, 'DD-MM-YYYY'), com.descripcion, 
            tip.id_unico, UPPER(tip.codigo), LOWER(tip.nombre), 
            (SELECT SUM(dcp.valor) 
             FROM gf_detalle_comprobante_pptal dcp 
             WHERE dcp.comprobantepptal = com.id_unico) AS valor 
            FROM gf_comprobante_pptal com 
            LEFT JOIN gf_tipo_comprobante_pptal tip ON tip.id_unico = com.tipocomprobante 
            WHERE
            com.id_unico = $adicion");
            break;
        default:
    ...
    $idModificacion = $modif[0][0];
    $numeroMod      = $modif[0][1];
    $fecha          = $modif[0][2];
    $fechaVen       = $modif[0][3];
    $descripcion    = $modif[0][4];
    $idTipo         = $modif[0][5];
    $nombreTipo     = $modif[0][6] . ' - ' . ucwords($modif[0][7]);
} else {
    $adicion = "";
}
?>
                            ...
                            <div class="col-sm-12" align="left" style="padding-left: 0px;"> <!-- Fila Uno -->
                                <div class="col-sm-3" align="left" style="padding-left: 0px;">
                                    ...
                                    <select name="tipoComPtal" id="tipoComPtal" class="select2_single form-control input-sm" title="Seleccione un tipo de comprobante" style="width: 180px;" required>
                                        ...
                                        <?php } else { ?>
                                            <?php $tipoComPtal = $con->Listar("SELECT id_unico, UPPER(codigo), LOWER(nombre)        
                                            ...
                                            <?php
                                            for ($row = 0; $row < count($tipoComPtal); $row++) {
                                                echo '<option value="' . $tipoComPtal[$row][0] . '">' . $tipoComPtal[$row][1] . ' ' . ucwords(($tipoComPtal[$row][2])) . '</option>';
                                            }
                                            ?>
                                        <?php } ?>
                                    </select>
                                ...
                                <div class="col-sm-3" align="left" style="padding-left: 0px;">
                                    <div style="width: 150px;">
                                        <label for="noDisponibilidad" class="control-label">
                                    ...
                                    <select class="select2_single form-control" name="buscarDisp" id="buscarDisp" style="width:250px">
                                        ...
                                        switch ($conexion) {
                                            ...
                                            case 2:
                                                ...
                                                tcp.codigo,
                                                CASE
                                                    WHEN (tr.nombreuno || tr.nombredos || tr.apellidouno || tr.apellidodos) IS NULL
                                                         OR (tr.nombreuno || tr.nombredos || tr.apellidouno || tr.apellidodos) = '' THEN tr.razonsocial
                                                    ELSE (tr.nombreuno || tr.nombredos || tr.apellidouno || tr.apellidodos)
                                                END AS NOMBRE,
                                                tr.numeroidentificacion
                                                FROM
                                                    gf_comprobante_pptal cp
                                                ...
                                                WHERE
                                                    tcp.clasepptal = 13
                                                    AND tcp.tipooperacion = 2
                                                    AND cp.parametrizacionanno = $anno
                                                    AND tcp.vigencia_actual = 1
                                                ORDER BY
                                                    cp.numero DESC");
                                                break;
                                            default:
                                        ...
                                        for ($row1 = 0; $row1 < count($reg); $row1++) {
                                            $date = new DateTime($reg[$row1][2]);
                                            $f = $date->format('d/m/Y');
                                            $rowV = $con->Listar("SELECT SUM(dc.valor) 
                                            ...
                                            WHERE comprobantepptal = " . $reg[$row1][0]);
                                            $v = ' $' . number_format($rowV[0][0], 2, '.', ',');
                                        ?>
                                            <option value="<?php echo $reg[$row1][0] ?>"><?php echo $reg[$row1][1] . ' ' . mb_strtoupper($reg[$row1][3]) . ' ' . $f . ' ' . $v ?>
                                            <?php } ?>
                                    </select>
                                </div>
                                <!-- BUSQUEDAS -->
                                <script type="text/javascript">
                                    $("#buscarDisp").change(function() {
                                        if (($("#buscarDisp").val() != "") && ($("#buscarDisp").val() != 0)) {
                                            ...
                                            document.location = 'ADICION_APROPIACION.php?id=' + btoa(id) ;
                                        }
                                    });
                                </script>
                                <!-- Botón Nuevo -->
                                <div class="col-sm-2" style="margin-left:-60px">
                                    ...
                                    <?php if(isset($_REQUEST['id'])){ ?>
                                        <button type="submit" id="btnGuardarElComp" class="btn btn-primary sombra" style="background: #00548F; color: #fff; border-color: #1075C1;  margin: 0 auto;" title="Guardar" disabled>
                                                    ...
                                                    success: function(response) {
                                                        ...
                                                        if (response != 2) {
                                                            ...
                                                            $('#btnExitoElComp').click(function() {
                                                                document.location = 'ADICION_APROPIACION.php?id=' + btoa(response);
                                                            });
                                                        } else {
                                    ...
                                    <script type="text/javascript">
                                        $(document).ready(function() {
                                            $("#btnImprimir").click(function() {
                                                var id = $("#id").val();
                                                window.open('informesPptal/inf_Adic_Aprop.php?id=' + btoa(id));
                                            });
                                        });
                                        $(document).ready(function() {
                                            $("#btnImprimirExcel").click(function() {
                                                var id = $("#id").val();
                                                window.open('informesPptal/inf_Adic_ApropExcel.php?id=' + btoa(id));
                                            });
                                        });
                                    </script>
                                <?php } ?>
                            ...
                            <div class="row" style="margin-top: 0px;">
                                ...
                                <div id="divRubro" class="form-group form-inline col-sm-3" style="margin-top: 5px; margin-left: 5px;" align="left">
                                    ...
                                    <select name="rubro" id="rubro" requiered="required" class=" form-control input-sm select2_single" title="Seleccione el rubro" style="width:150px;">
                                        ...
                                        switch ($conexion) {
                                            case 1:
                                                $rubro = $con->Listar("SELECT id_unico, CONCAT(codi_presupuesto, ' ',nombre) rubro 
                                                FROM gf_rubro_pptal WHERE movimiento = 1 
                                                AND parametrizacionanno = $anno 
                                                ORDER BY codi_presupuesto ASC");
                                                break;
                                            case 2:
                                                $rubro = $con->Listar("SELECT id_unico, codi_presupuesto || ' ' || nombre AS rubro
                                                FROM gf_rubro_pptal
                                                WHERE movimiento = 1
                                                AND parametrizacionanno = $anno 
                                                ORDER BY codi_presupuesto ASC");
                                        ...
                                        for ($rowRub = 0; $rowRub < count($rubro); $rowRub++) {  ?>
                                            <option value="<?php echo $rubro[$rowRub][0]; ?>"><?php echo ucwords(mb_strtolower($rubro[$rowRub][1])); ?></option>
                                        <?php }
                                        ?>
                                    </select>
                                </div>
                        ...
                        <table id="tabla" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                if (!empty($adicion)) {
                                    ...
                                    switch ($conexion) {
                                        case 1:
                                            $resultado = $con->Listar("SELECT detCompP.id_unico, 
                                            ...
                                            WHERE compPtal.id_unico = $id
                                            ORDER BY detCompP.id_unico DESC");
                                            break;
                                        case 2:
                                            $resultado = $con->Listar("SELECT detCompP.id_unico,
                                            rubP.codi_presupuesto || '- ' || rubP.nombre,
                                            fue.id_unico || '- ' || fue.nombre,
                                            detCompP.valor,
                                            detCompP.rubrofuente
                                            FROM gf_detalle_comprobante_pptal detCompP
                                            LEFT JOIN gf_rubro_fuente rubFue ON rubFue.id_unico = detCompP.rubrofuente
                                            LEFT JOIN gf_fuente fue ON fue.id_unico = rubFue.fuente
                                            LEFT JOIN gf_rubro_pptal rubP ON rubP.id_unico = rubFue.rubro
                                ...
                                ?>
                                        <tr>
                                            <td class="oculto"><?php echo $resultado[$row][0] ?></td>
                                            <td class="campos">
                                                <a class="eliminar" href="#<?php echo $resultado[$row][0]; ?>" onclick="javascript:verificarValorEliminar(<?php echo $resultado[$row][0] . ',' . $resultado[$row][4]; ?>);">
                                                    <i title="Eliminar" class="glyphicon glyphicon-trash">
                                                    </i>
                                                </a>
                                                <a class="modificar" href="#<?php echo $resultado[$row][0]; ?>" onclick="javascript:modificarDetComp(<?php echo $resultado[$row][0]; ?>);">
                                                    <i title="Modificar" class="glyphicon glyphicon-edit"></i>
                                                </a>
                                            </td>
                                            <td class="campos" align="left">
                                                <?php echo ucwords(mb_strtolower($resultado[$row][1])); ?>
                                            </td>
                                            <td class="campos" align="left">
                                                <?php echo ucwords(mb_strtolower($resultado[$row][2])); ?>
                                            </td>
                                            <td class="campos" align="right">
                                                <input type="hidden" id="valOcul<?php echo $resultado[$row][0]; ?>" value="<?php echo number_format($resultado[$row][3], 2, '.', ','); ?>">
                                                <div id="divVal<?php echo $resultado[$row][0]; ?>">
                                                    <?php
                                                    
                                                    $valor = str_replace(',', '.', $resultado[$row][3]);
                                                    echo number_format($valor,2);
                                                    ?>
                                                </div>
                                                <!-- Modificar los valores -->
                                                <table id="tab<?php echo $resultado[$row][0]; ?>" style="padding: 0px;  margin-top: -10px; margin-bottom: -10px;">
                                                    <tr>
                                                        <td>
                                                            <input type="hidden" name="valorMod" id="verificar<?php echo $resultado[$row][0]; ?>" value="<?php echo $validar ?>">
                                                            <input type="text" name="valorMod" id="valorMod<?php echo $resultado[$row][0]; ?>" class="fo9rm-control in9put-sm" maxlength="50" style="width:150px; margin-top: -5px; margin-bottom: -5px; " placeholder="Valor" onkeypress="return txtValida(event,'dec', 'valorMod<?php echo $resultado[$row][0]; ?>', '2');" onkeyup="formatC('valorMod<?php echo $resultado[$row][0]; ?>')" value="<?php echo number_format($resultado[$row][3], 2, '.', ','); ?>" required>
                                                        </td>
                                                        <td>
                                                            <a href="#<?php echo $resultado[$row][0]; ?>" onclick="javascript:verificarValorModificar('<?php echo $resultado[$row][0]; ?>','<?php echo $resultado[$row][4] ?>');">
                                                                <i title="Guardar Cambios" class="glyphicon glyphicon-floppy-disk"></i>
                                                            </a>
                                                        </td>
                                                        <td>
                                                            <a href="#<?php echo $resultado[$row][0]; ?>" onclick="javascript:cancelarModificacion(<?php echo $resultado[$row][0]; ?>);">
                                                ...
                                                <script type="text/javascript">
                                                    var id = "<?php echo $resultado[$row][0]; ?>";
