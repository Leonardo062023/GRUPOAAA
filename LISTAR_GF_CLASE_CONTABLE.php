...
switch ($_SESSION['conexion']) {
    case '1':
      $row = $con->Listar("SELECT C.Id_Unico,C.Nombre,ASOC.Nombre FROM gf_clase_contable C LEFT JOIN gf_clase_contable ASOC
ON C.ClaseAso = ASOC.Id_Unico ORDER BY C.Nombre ASC");
    break;
    case '2':
      $row = $con->Listar("SELECT C.Id_Unico,C.Nombre,ASOC.Nombre FROM gf_clase_contable C LEFT JOIN gf_clase_contable ASOC
      ON C.ClaseAso = ASOC.Id_Unico ORDER BY C.Nombre ASC");
          ...
          <table id="tabla" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
            ...
            <?php
               for ($i=0; $i <count($row) ; $i++) { 
                ...
                <td><a href="#" onclick="javascript:eliminarContrato(<?php echo $row[$i][0];?>);"><i title="Eliminar" class="glyphicon glyphicon-trash"></i></a><a href="EDITAR_GF_CLASE_CONTABLE.php?id_clase=<?php echo($row[$i][0]);?>"><i title="Modificar" class="glyphicon glyphicon-edit" ></i></a></td>
      ...
      function eliminarContrato(id)
      {
        $("#myModal").modal('show');
                        $("#ver").click(function(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificarClaseContableJson.php?action==1",
                                data: form_data,
