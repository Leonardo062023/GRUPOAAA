...
switch ($_SESSION['conexion']) {
    case '1':
      $row = $con->Listar("SELECT id_unico, fecha, descripcion FROM gf_festivos WHERE (id_unico)='$id'");
    break;
    case '2':
      $row = $con->Listar("SELECT id_unico, TO_CHAR(fecha,'DD/MM/YYYY') fecha, descripcion FROM gf_festivos WHERE (id_unico)='$id'");
  break;
  }
?>
    ...
    <?php require_once 'menu.php'; ?>
        ...
        <h2 class="tituloform" align="center" >Modificar Festivos</h2>
            ...
            <form name="form" id="form" class="form-horizontal" method="POST"  enctype="multipart/form-data" action="javascript:modificar()">
                ...
                <input type="hidden" name="id" id="id" value="<?php echo $row[$i][0];?>">
                    ...
                    <div class="form-group" style="margin-top: -20px;">
                        ...
                        <textarea type="text" name="descripcion" id="descripcion" class="form-control col-sm-1" onkeypress="return txtValida(event)" maxlength="500" title="Ingrese descripción"  placeholder="Descripción" style="margin-top:0.1em; height: 65px;" required="required" ><?php echo ucwords(mb_strtolower($row[$i][2]))?></textarea>
                    </div>
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificar_GF_FESTIVOSJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
