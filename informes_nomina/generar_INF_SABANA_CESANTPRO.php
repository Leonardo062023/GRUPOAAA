...
$fechaInicio = $np[0][4];
$andw = "";
if(!empty($_REQUEST['sltTipoE']) ){
    $tipoe = $_REQUEST['sltTipoE'];
    $andw .= " AND te.tipo = $tipoe ";
    
}
if(!empty($_REQUEST['sltUnidadE']) ){
    $unidad = $_REQUEST['sltUnidadE'];
    $andw .= " AND e.unidadejecutora = $unidad ";
}
if(!empty($_REQUEST['sltRegimenC'])){
    $regimen = $_REQUEST['sltRegimenC'];
    $andw .= " AND e.cesantias = $regimen ";
}
#**** Numero de conceptos ****#
$ncon = $con->Listar("SELECT COUNT(DISTINCT n.concepto) 
    ...
    LEFT JOIN gn_empleado_tipo te ON te.empleado = e.id_unico 
...
if($_GET['t']==1){
    if ($grupoG==1) {
        ...
        { 
            ...
            { 
                ...
                $this->Ln(4);
                $this->SetX(25);
                $this->Cell(320, 5, utf8_decode('SÁBANA DE CESANTÍAS'), 0, 0, 'C');
                $this->Ln(4); 
                $this->SetFont('Arial', 'B', 8);
                $this->SetX(25);
        ...
        if(!empty($_REQUEST['sltRegimenC'])){
            $tipo = $con->Listar("SELECT nombre from gn_regimen_cesantias WHERE id_unico =".$_REQUEST['sltRegimenC']);
            $pdf->SetX(25);
            $pdf->Cell(298,5, utf8_decode('TIPO RÉGIMEN: '.$tipo[0][0]),1,0,'L');
            $pdf->Ln(5);
        }
    ...
    for ($g = 0; $g < count($rowg); $g++) {
        ...
        if ($rowcn[0]!=null) {
            ...
            $pdf->Cell(17,5, utf8_decode('Fecha Ingreso'),0,0,'C');
        ...
        $pdf->Cell(17,$alto, utf8_decode(''),1,0,'C');
        ...
        $andw 
        ORDER BY CONCAT_WS(' ',t.apellidouno,t.apellidodos,t.nombreuno,t.nombredos) DESC ");
        $pdf->SetFont('Arial','',7);
        $salarioa = 0;
        for ($e = 0; $e < count($rowe); $e++) { 
            ...
            $pdf->Cellfitscale(17,8, ($rowe[$e][7]),1,0,'L');
...
$pdf->SetFont('Arial','B',8);
$pdf->SetX(25);    
$pdf->Cell(80,8, utf8_decode('Total Final:'),1,0,'C');
$pdf->Cellfitscale(18,8, utf8_decode(number_format($salarioa,0,'.',',')),1,0,'R');
...
$pdf->Cellfitscale(22.22,8, number_format($valor, 0, '.',','),1,0,'R');  
}
    $pdf->Ln(10);
    ...
    ob_end_clean();
    $pdf->Output(0,'Sabana_Cesantias('.date('d/m/Y').').pdf',0);  
}else{
    require'../fpdf/fpdf.php';
    ob_start();
    class PDF extends FPDF
    ...
    if(!empty($_REQUEST['sltTipoE']) ){
        $tipo = $con->Listar("SELECT nombre from gn_tipo_empleado WHERE id_unico =".$_REQUEST['sltTipoE']);
        $pdf->SetX(35);
        $pdf->Cell(295,5, utf8_decode('TIPO EMPLEADO: '.$tipo[0][0]),1,0,'L');
        $pdf->Ln(5);        
    }
    if(!empty($_REQUEST['sltUnidadE']) ){
        $tipo = $con->Listar("SELECT nombre from gn_unidad_ejecutora WHERE id_unico =".$_REQUEST['sltUnidadE']);
        $pdf->SetX(35);
        $pdf->Cell(295,5, utf8_decode('UNIDAD EJECUTORA: '.$tipo[0][0]),1,0,'L');
        $pdf->Ln(5);
    }
    if(!empty($_REQUEST['sltRegimenC'])){
        $tipo = $con->Listar("SELECT nombre from gn_regimen_cesantias WHERE id_unico =".$_REQUEST['sltRegimenC']);
        $pdf->SetX(35);
        $pdf->Cell(312,5, utf8_decode('TIPO RÉGIMEN: '.$tipo[0][0]),1,0,'L');
        $pdf->Ln(5);
    }
    $pdf->SetX(35);
    $cx = $pdf->GetX();
    $cy = $pdf->GetY();
    $pdf->Cell(10,5, utf8_decode('Item'),0,0,'C');
    $pdf->Cell(15,5, utf8_decode('Cédula'),0,0,'C');
    ...
    for ($e = 0; $e < count($rowe); $e++) { 
        $pdf->SetX(35);
        $pdf->Cellfitscale(10,8, $e+1,1,0,'L');
        $pdf->Cellfitscale(15,8, utf8_decode($rowe[$e][4]),1,0,'L');
        $pdf->Cellfitscale(45,8, ($rowe[$e][5]),1,0,'L');
        $pdf->Cellfitscale(17,8, ($rowe[$e][4]),1,0,'L');
    ...
    $pdf->SetX(35);
    $pdf->SetFont('Arial','B',8);
    $pdf->Cell(87,8, utf8_decode('Total:'),1,0,'C');
   ...
   for ($c = 0; $c < count($rowcn); $c++) {
    ...
    LEFT JOIN gf_cargo c ON ct.cargo = c.id_unico
    LEFT JOIN gf_tipo_documento td ON rd.tipodocumento = td.id_unico
    WHERE td.nombre = 'Sabana Cesantias'
    ORDER BY rd.orden ASC";
...
#******** Tipo Excel *************#
     ...
     if ($grupoG==1) {
        ...
        header("Content-Disposition: attachment; filename=Sabana_LiquidacionF.xls");  
        ...
        if(!empty($_REQUEST['sltTipoE']) ){
            $tipo = $con->Listar("SELECT nombre from gn_tipo_empleado WHERE id_unico =".$_REQUEST['sltTipoE']);
            echo '<tr><td colspan="'.$nc.'" align="center"><strong><i>TIPO EMPLEADO: '.$tipo[0][0].'</i></strong></td></tr>';
        }
        if(!empty($_REQUEST['sltUnidadE']) ){
            $tipo = $con->Listar("SELECT nombre from gn_unidad_ejecutora WHERE id_unico =".$_REQUEST['sltUnidadE']);
            echo '<tr><td colspan="'.$nc.'" align="center"><strong><i>UNIDAD EJECUTORA: '.$tipo[0][0].'</i></strong></td></tr>';
        }
        if(!empty($_REQUEST['sltRegimenC'])){
            $tipo = $con->Listar("SELECT nombre from gn_regimen_cesantias WHERE id_unico =".$_REQUEST['sltRegimenC']);
            echo '<tr><td colspan="'.$nc.'" align="center"><strong><i>TIPO RÉGIMEN: '.$tipo[0][0].'</i></strong></td></tr>'; 
        }
        ...
        for ($e = 0; $e < count($rowe); $e++) { 
            ...
            echo '<td align= "left">'.($rowe[$e][7]).'</td>';
            #*** Salario ****#
            $basico = $con->Listar("SELECT 
                valor FROM gn_novedad 
                WHERE empleado = ".$rowe[$e][0]." 
        ...
        echo '<tr>';
        echo '<td colspan="4" ><strong>Total</strong></td>';
        echo '<td   align="right"><strong>'.number_format($salarioa,0,'.',',').'</strong></td>';
        ...
        for ($c = 0; $c < count($rowcn); $c++) {
            ...
            echo '<td align="right" ><strong>'.number_format($valor,0,'.',',').'</strong></td>'; 
        }
          echo '</tr>';
