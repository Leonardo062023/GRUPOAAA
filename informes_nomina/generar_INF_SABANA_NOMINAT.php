...
if($_GET['t']==1){
    ...
    if(empty($_REQUEST['sltUnidadE']) && empty($_REQUEST['sltTipoE'])){        
        ...
        $pdf->Cell(10,5, utf8_decode('Básico'),0,0,'C');
        $h2 = 0;
        $h  = 0;
        $alto = 0;
        #**** Nombre de conceptos ****#
        ...
        for ($e = 0; $e < count($rowe); $e++) { 
            ...
            $pdf->Cellfitscale(10,8, utf8_decode(number_format($rowe[$e][6],0,'.',',')),1,0,'R');
            $salarioa += $rowe[$e][6];
            $x =$pdf->GetX();  
            $y =$pdf->GetY();
            for ($c = 0; $c < count($rowcn); $c++) {
        ...
        $pdf->Cellfitscale(10,8, utf8_decode(number_format($salarioa,0,'.',',')),1,0,'R');
       ...
       for ($c = 0; $c < count($rowcn); $c++) {
        ...
        $pdf->Cellfitscale(10,8, utf8_decode(number_format($totalsalarioa,0,'.',',')),1,0,'R');
...
#******** Tipo Excel *************#
    ...
    if(empty($_REQUEST['sltUnidadE']) && empty($_REQUEST['sltTipoE'])){
        ...
        echo '<td><strong>'.number_format($totalsalarioa,0,'.',',').'</strong></td>';
        ...
        if($acumulado ==1 ){
            ...
            echo '<td><strong><i>'.number_format($totalsa,0,'.',',').'</i></strong></td>';
