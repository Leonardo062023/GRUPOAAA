...
//validacion preguntando si la variable enviada del listar viene vacia
  if (isset($_GET["id_tipoR"])){ 
    $id_tipoR= (($_GET["id_tipoR"]));
//Query o sql de consulta
switch ($_SESSION['conexion']) {
  case '1':
    $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_tipo_responsable  WHERE md5(Id_Unico) ='$id_tipoR'");
  break;
  case '2':
    $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_tipo_responsable  WHERE (Id_Unico) ='$id_tipoR'");
break;
}
}
?>
  <title>Modificar Tipo Responsable</title>
</head>
...
<!-- Llamado al menú del formulario -->   
    ...
    <div class="col-sm-10 text-left">
      <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;">Modificar Tipo Responsable</h2>
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificarTiporesponsableJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
