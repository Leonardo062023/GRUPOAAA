...
switch ($_SESSION['conexion']) {
  case '1':
    $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_tipo_operacion  WHERE md5(Id_Unico) ='$id_tipo'");
  break;
  case '2':
    $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_tipo_operacion  WHERE (Id_Unico) ='$id_tipo'");
break;
}
}
?>
  <title>Modificar Tipo Operacion</title>
</head> 
...
<!-- Llamado al menú del formulario -->    
    ...
    <div class="col-sm-10 text-left">
      ...
      <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;">Modificar Tipo Operación</h2>
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificarTipoOperJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
