...
if(!empty($_POST['txtPeridoF'])){
    ...
    for ($i = 0; $i < count($pros); $i++) {
        ...
        $siglaEnt   = $pro->obtenerTipoMovimientoEntrada($id_unico);
        $siglaSal   = $dep->obtenerTipoMovimientoSalida($id_unico);
        if ($siglaEnt=='EDI' || $siglaSal=='SDI' ) {
           $fechaConv=$fecha;
        }else{
           $fechaConv=$fechaS;
        }
          $fecha_div1 = explode("/", $fechaConv);
          $anio1 = $fecha_div1[2];
          $mes1 = $fecha_div1[1];
         ...
         $diff = (($year2 - $anio1) * 12) + ($month2 - $mes1);
         $valorMesAcum  = $dep->obtenerMesesAcumDepreciacion($id_unico);
        ...
        $html .= "\n\t\t\t\t<td align=\"right\">$vidaU</td>";
        $html .= "\n\t\t\t\t<td align=\"right\">".number_format($mes,2,',','.')."</td>";
        $html .= "\n\t\t\t\t<td align=\"right\">"."$valorMesAcum</td>";
    ...
    $html .= "\n\t\t\t\t<td align=\"right\"><b>".number_format($totalM,2,',','.')."</b></td>";
    $html .= "\n\t\t\t\t<td colspan=\"1\" align=\"right\"></td>";
