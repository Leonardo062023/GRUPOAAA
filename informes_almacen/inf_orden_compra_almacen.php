...
$pdf->Ln(10);
    ...
    $pdf->Cell(70, 5, utf8_decode('PLAN'), 'LRT', 0, 'C');
    $pdf->Cell(30, 5, '', 'LRT', 0, 'C');
    $pdf->Cell(30, 5, '', 'LRT', 0, 'C');
    $pdf->Cell(30, 5, 'VALOR', 'LRT', 0, 'C');
    $pdf->Cell(30, 5, '', 'LRT', 0, 'C');
    $pdf->Ln(5);
    $pdf->Cell(10, 5, utf8_decode('Nª'), 'LRB', 0,'C');
    $pdf->Cell(70, 5, 'INVENTARIO', 'LRB', 0, 'C');
    $pdf->Cell(30, 5, 'UNIDAD', 'LRB', 0, 'C');
    $pdf->Cell(30, 5, 'CANTIDAD', 'LRB', 0, 'C');
    $pdf->Cell(30, 5, 'UNITARIO', 'LRB', 0, 'C');
    ...
    while ($rowP = mysqli_fetch_row($resultP)) {
        ...
        $pdf->Ln(5);
        $pdf->SetFont('Arial', '', 8);
        $pdf->SetWidths(array(10, 70,30,  30, 30, 30));
        $pdf->SetAligns(array('C', 'L','L', 'C',  'R', 'R'));
        $pdf->Row(array($a,utf8_decode(mb_strtoupper($rowP[1])), $unid, number_format($rowP[2], 2), $valorA, $valorTA));
        if($rowP[5] == 2){
            $devoltivos[] = $rowP[0];
        }
    }
