...
switch ($_SESSION['conexion']) {
    case '1':
      $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_actividad_mantenimiento WHERE Id_Unico = '$id_act_man'");
    break;
    case '2':
      $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_actividad_mantenimiento WHERE Id_Unico = '$id_act_man'");
    break;
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificar_GF_ACTIVIDAD_MANTENIMIENTOJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
