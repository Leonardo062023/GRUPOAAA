...
$resultado = $con->Listar("SELECT ef.Id_Unico id, ef.Nombre Nombre, td.Nombre tipoDato FROM gf_elemento_ficha ef, gf_tipo_dato td  WHERE ef.TipoDato=td.Id_Unico");
                            ...
                            <tbody>
                                ...
                                for ($row=0; $row < count($resultado); $row++) {  ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#" onclick="javascript:eliminarElementoFicha(<?php echo $resultado[$row][0]; ?>);"><i title="Eliminar" class="glyphicon glyphicon-trash"></i></a>
                                            <a href="modificar_GF_ELEMENTO_FICHA.php?action=1&id_elemento_ficha=<?php echo base64_encode($resultado[$row][0]); ?>"><i title="Modificar" class="glyphicon glyphicon-edit"></i></a>
                                        </td>
                                        <td><?php echo utf8_encode($resultado[$row][1]) ?></td>
                                        <td><?php echo utf8_encode($resultado[$row][2]) ?></td>
                                    </tr>
                                <?php } ?>
                            </tbody>
                        </table>
                        <div align="right"><a href="modificar_GF_ELEMENTO_FICHA.php?action=2" class="btn btn-primary sombra" style="box-shadow: 0px 2px 5px 1px gray;color: #fff;  border-color: #1075C1; margin-top: 20px; margin-bottom: 20px; margin-left:-20px; margin-right:4px">Registrar Nuevo</a>
                        </div>
                    </div>
                </div>
            </div>
