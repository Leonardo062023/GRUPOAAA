...
if (!empty($_SESSION['id_comp_pptal_GE'])) {
    ...
    $queryGen = "SELECT detComP.id_unico, 
        CONCAT(rub.codi_presupuesto,' - ',rub.nombre), detComP.valor, 
        rubFue.id_unico, fue.nombre  ,cc.id_unico, cc.nombre , 
        t.id_unico, 
        IF(CONCAT_WS(' ',
            t.nombreuno,
            t.nombredos,
      ...
      LEFT JOIN gf_centro_costo cc ON cc.id_unico =detComP.centro_costo  
      LEFT JOIN 
    ...
    $queryCompro = "SELECT comp.id_unico, comp.numero, comp.fecha, comp.descripcion, 
        comp.fechavencimiento, comp.tipocomprobante, tipCom.codigo, tipCom.nombre, comp.tercero, 
        IF(CONCAT_WS(' ',
            t.nombreuno,
            t.nombredos,
            t.apellidouno,
      ...
      LEFT JOIN gf_tercero t ON comp.tercero = t.id_unico 
      WHERE comp.id_unico = " . $_SESSION['id_comp_pptal_GE'];
    ...
    if (mysqli_num_rows($comprobante) > 0) {
        ...
        if (!empty($_SESSION['nuevo_GE'])) {
            ...
            $_SESSION['idCompCntV'] = $comprobateCnt;
        }
    }
}
$conciliado = 0;
...
if(!empty($_SESSION['idCompCntV'] )){  
  $conciliado = conciliadocnt($_SESSION['idCompCntV']);
} 
...
$querySolAprob = "SELECT comp.id_unico, comp.numero, comp.fecha, comp.descripcion       
    ...
    WHERE tipcomp.clasepptal = 15 AND compania = $compania  
    AND comp.estado = 3
    OR comp.estado = 4
    ORDER BY comp.numero";
$SolAprob = $mysqli->query($querySolAprob);

$queryTercero = "SELECT tr.id_unico, IF(CONCAT_WS(' ',
     tr.nombreuno,
     tr.nombredos,
     tr.apellidouno,
     tr.apellidodos) 
     ...
     tr.nombredos,
     tr.apellidouno,
     tr.apellidodos)) AS NOMBRE, tr.numeroidentificacion, perTer.perfil     
    FROM gf_tercero tr 
    LEFT JOIN gf_perfil_tercero perTer ON perTer.tercero = tr.id_unico 
    WHERE tr.id_unico != $idte  AND tr.compania = $compania 
...
<body>
    ...
    <input type="hidden" id="respuesta" value="<?php
    if (empty($_SESSION['nuevo_GE'])) {
        if (!empty($_SESSION['id_comp_pptal_GE'])) {
    ...
    <div class="container-fluid text-center"  >
        <div class="row content">
            ...
            <div class="col-sm-10" style="margin-left: -16px;margin-top: 5px" > 
                <h2 align="center" class="tituloform col-sm-12 col-xs-12 col-md-12 col-lg-12" style="margin-top: -5px; margin-bottom: 2px;" >Generar Egreso</h2>
                <div class="col-sm-12"><!--   estaba 10 -->
                    <div class="client-form contenedorForma col-sm-12 col-xs-12	col-md-12 col-lg-12"  style="padding: 0px;"> 
                        ...
                        <form name="form" class="form-horizontal" method="POST"  enctype="multipart/form-data"  action="json/registrar_GENERAR_EGRESOJsonL.php"> 
                            ...
                            <div class="form-group form-inline col-sm-12 col-xs-12 col-md-12 col-lg-12" style="margin-top: 0px; margin-left: 0px; margin-bottom: 0px;">
                                <!----**** Tercero *****--->
                                <div class="col-sm-2 col-xs-2 col-md-2 col-lg-2" align="left"> <!-- Tercero -->
                                    ...
                                    <select name="tercero" id="tercero" class="select2_single form-control input-sm" title="Seleccione un tipo de comprobante" style="width:150px; height: 30px" required onchange="llenar();">
                                        <?php if (!empty($_SESSION['nuevo_GE']) && !empty($_SESSION['id_comp_pptal_GE'])) {
                                            echo '<option value="'.$rowComp[8].'">'.ucwords(mb_strtolower($rowComp[9])).' - '.$rowComp[10].'</option>';
                                        } elseif(!empty($_SESSION['id_comp_pptal_GE'])) {
                                            echo '<option value="'.$rowComp[8].'">'.ucwords(mb_strtolower($rowComp[9])).' - '.$rowComp[10].'</option>';
                                        } else {
                                            echo '<option value="">Tercero</option>';
                                        }
                                        while ($rowTerc = mysqli_fetch_row($tercero)) {
                                            echo '<option value="'.$rowTerc[0].'">'.ucwords(mb_strtolower($rowTerc[1])) . ' ' . $rowTerc[2].'</option>';
                                        }
                                        ?>
                                    </select>
                                </div> 
                                <!-- Fin Tercero -->
                                <!----**** Cuenta Por Pagar *****--->
                                <div class="col-sm-2 col-xs-2 col-md-2 col-lg-2" align="left"> <!-- Registro Presupuestal -->
                                    ...
                                    <select name="solicitudAprobada" id="solicitudAprobada" class="select2_single form-control input-sm" title="Cuenta por Pagar" style="width:150px; height: 30px" >
                                        ...
                                        if ((!empty($_SESSION['terceroGuardado']))) {
                                            ...
                                            $queryComp = "SELECT  com.id_unico, com.numero, com.fecha, com.descripcion
                                                ...
                                                WHERE tipoCom.clasepptal = 16 and com.tercero =  $id_tercero AND com.parametrizacionanno = $anno";
                                ...
                                <!----**** Tipo Comprobante *****--->
                                <div class="col-sm-2 col-xs-2 col-md-2 col-lg-2" align="left" > <!-- Tipo Comprobante -->
                                    ...
                                    <select name="tipoComprobante" id="tipoComprobante" class="form-control input-sm" title="Tipo Comprobante" style="width:150px; height: 30px" required>
                                        <?php
                                        echo $_SESSION['id_comp_pptal_GE'];
                                ...
                                <script>
                                        $(document).ready(function ()
                                        {
                                            form_data = {action: 2, id: $("#idComPtal").val()}
                                            $.ajax({
                                                type: "POST",
                                ...
                                <?php } ?>
                                <div class="col-sm-2 col-xs-2 col-md-2 col-lg-2" align="left"><!--  Fecha -->
                                ...
                                <div class="col-sm-2 col-xs-2 col-md-2 col-lg-2 " align="left" style="margin-left:-50px;"><!--  Check Egreso -->
                                    <div id="tipo" style="display:none">
                                        <label class="control-label"><strong style="color:#03C1FB;">*</strong>Tipo Egreso:</label> <br/>
                                        <label class="control-label"  align="left" style="width: 80px; margin-top: -10px"><strong style="color:#03C1FB;">*</strong>Aportes:</label> 
                                        <input type="checkbox" id="aportes" value="aportes"/><br/>
                                        <input type="hidden" name="id_aportes" id="id_aportes">
                                        <label class="control-label" style="width: 80px; margin-top: -10px"><strong style="color:#03C1FB;">*</strong>Devengos:</label> 
                                        <input type="checkbox" value="devengos"  id="devengos"/>
                                        <input type="hidden" name="id_devengos" id="id_devengos">
                                    </div>
                                    <script>
                                        $(document).on('change', 'input[type="checkbox"]', function (e) {
                                            if (this.id == "aportes") {
                                                if (this.checked)
                                                    $('#id_aportes').val(this.value);
                                                else
                                                    $('#id_aportes').val("");
                                            }
                                            if (this.id == "devengos") {
                                                if (this.checked)
                                                    $('#id_devengos').val(this.value);
                                                else
                                                    $('#id_devengos').val("");
                                            }
                                        });
                                ...
                                <div class="col-sm-1" style="margin-top: 22px;margin-left:-50px;"> <!-- Botón guardar -->
                                ...
                                <div class="col-sm-1" style="margin-top: 22px;margin-left:-30px;"> <!-- Botón siguiente -->
                                ...
                                <div class="col-sm-1" style="margin-top: 22px;margin-left:-30px;"> <!-- Botón ver formulario registro_COMPROBANTE_CNT.php -->
                                ...
                                <div class="col-sm-1" style="margin-top: 22px;margin-left:-30px;"> <!-- Botón nuevo -->
                            ...
                            <div class="form-group form-inline col-sm-12" style="margin-top: 0px; margin-left:2px; margin-bottom: 5px;"> <!-- Segunda Fila -->
                                <div class="col-sm-3" align="left" >
                                    <label for="numEgreso" class="control-label" style="">No. Egreso:</label><br>
                                    <input class="input-sm" type="text" name="numEgreso" id="numEgreso" class="form-control" style="width:150px; margin-top: 0px; margin-bottom: 0px; height: 30px" title="Número de comprobante de Egreso" placeholder="Número Egreso" readonly="readonly" value="<?php
if (!empty($_SESSION['nuevo_GE'])) {
    echo $numero;
}
?>"> <!---->
                                ...
                                <div class="col-sm-4" style="margin-top: -2px; margin-left:-90px" > <!-- Buscar disponibilidad -->
                                    ...
                                    <select class="select2_single form-control" name="buscarReg" id="buscarReg" style="width:450px; height: 30px">
                                        ...
                                        $reg = "SELECT
                                              ...
                                              WHERE tcp.clasepptal = 17 AND tcp.tipooperacion = 1 AND cp.parametrizacionanno = $anno ORDER BY cp.numero DESC";
...
<?php } ?>
                                ...
                                <div class="col-sm-2 col-xs-2 col-md-2 col-lg-2 " align="left" style="margin-left:-50px;"><!--  Check Egreso -->
                                    ...
                                    <?php }  elseif (!empty($_SESSION['nuevo_GE']) && !empty($_SESSION['id_comp_pptal_GE'])) { 
                                            #* Contar Detalles 
                                            $dt = detallesnumpptal($_SESSION['id_comp_pptal_GE']);
                                            if($dt>0){ ?>
                                            <div id="vigencia" style="display:block">
                                            <?php } else { ?>        
                                            <div id="vigencia" style="display:block">
                                    ...
                                    <?php } ?>    
                                        <br/>
                                        <label class="control-label"  align="left" style="width: 80px; margin-top: -10px"><strong style="color:#03C1FB;">*</strong>Vigencia Anterior:</label> 
                                                    ...
                                                    if (this.checked){
                                                        if(ter==""){
                                                           ...
                                                           var opcion = '<option value="" >Cuenta por Pagar</option>';
                                                            var form_data = {action:3, tercero: +$("#tercero").val()};
                                                            $.ajax({
                                                                type: "POST",
                                                                url: "jsonPptal/gf_egresoJson.php",
                                                                data: form_data,
                                ...
                                <!--  Imprimir.-->
                                <div class="col-sm-1" style="margin-top: 5px;margin-left: -40px;">
...
<?php } ?>           
                                ...
                                <div class="col-sm-1" id="agregarC" name="agregarC" style="margin-top: 5px; margin-left: -30px; display: inline-block" >
                                ...
                                <script type="text/javascript">
                                    $(document).ready(function ()
                                    {
                                        ...
                                        {
                                            jsShowWindowLoad('Agregando Cuenta Por Pagar...');
                                            ...
                                            /****Veriificar Fecha*****/
                                            var form_data ={action:7,fecha:fecha,cuentaxpagar:cuentaxpagar,comprobante_pptal:comprobante_pptal};
                                            $.ajax({
                                                type: "POST",
                                                url: "jsonPptal/gf_egresoJson.php",
                                                data: form_data,
                                                success: function (response)
                                                {
                                                    ...
                                                    if(response==1){
                                                        //Verificar Si es de Vigencia Anterior
                                                        if($("#vigencia").val()==1){
                                                            var form_data ={action:4};
                                                            $.ajax({
                                                                type: "POST",
                                                                url: "jsonPptal/gf_egresoJson.php",
                                                                data: form_data,
                                                                success: function (response)
                                                                {
                                                                    ...
                                                                    if(response==0){
                                                                        var form_data = {action:6, 
                                                                            ...
                                                                            fecha: fecha, 
                                                                            comprobantepptal: comprobante_pptal, 
                                                                            cxp : $("#solicitudAprobada").val(),
                                                                        };
                                                                        $.ajax({
                                                                            type: "POST",
                                                                            url: "jsonPptal/gf_egresoJson.php",
                                                                            ...
                                                                            {
                                                                                jsRemoveWindowLoad();
                                                                                console.log('Rta:GuardarAgregar:'+response);
                                                                        ...
                                                                        jsRemoveWindowLoad();
                                                                        $("#mensaje").html('No Existe Configuración De Cuentas Por Pagar Vigencia Anterior');
                                                                        $("#myModalError").modal("show");
                                                                         $("#btnErrorModal").click(function(){
                                                                             $("#myModalError").modal("hide");
                                                                        })
                                                            ...
                                                            success: function (response)
                                                            {
                                                                ...
                                                                if (response == 1)
                                                                {
                                                                    ...
                                                                    //VERIFICAR SI TIENE INTERFAZ
                                                                    var form_data = {estruc: 10, cxp: cuentaxpagar};
                                                                    $.ajax({
                                                                        type: "POST",
                                                                        ...
                                                                        success: function (response)
                                                                        {
                                                                            ...
                                                                            {
                                                                                /****Check Box***/
                                                                                var devengos = $("#id_devengos").val();
                                                                                var aportes = $("#id_aportes").val();
                                                                                ...
                                                                                $.ajax({
                                                                                    ...
                                                                                    success: function (response)
                                                                                    {
                                                                                        ...
                                                                                        if (Result == 2) {
                                                                                            ...
                                                                                            var form_data = {case: 3, fecha: fecha, cuentaxpagar: cuentaxpagar,
                                                                                                comprobante: comprobanteAnterior, tercero: tercero,
                                                                                                devengos: devengos, aportes: aportes};
                                                                                            $.ajax({
                                                                                                type: "POST",
                                                                                                url: "consultasBasicas/agregar_Cuenta_Pagar.php",
                                                                                                data: form_data,
                                ...
                                <div class="modal fade" id="mdlmsj" role="dialog" align="center" data-keyboard="false" data-backdrop="static">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div id="forma-modal" class="modal-header">
                                                <h4 class="modal-title" style="font-size: 24; padding: 3px;">Información</h4>
                                            ...
                                            <div class="modal-body" style="margin-top: 8px">
                                                <p>El egreso tiene cuentas conciliadas.<br/> No se puede eliminar.</p>
                                            </div>
                                            <div id="forma-modal" class="modal-footer">
                                                            ...
                                                            $.ajax({
                                                                ...
                                                                success: function (response)
                                                                {
                                                                    ...
                                                                    if (response == 1)
                                                                    {
                                                                        ...
                                                                        //VERIFICAR SI TIENE INTERFAZ
                                                                        var form_data = {estruc: 10, cxp: id_comprobante_pptal};
                                                                        $.ajax({
                                                                            type: "POST",
                                                                            url: "jsonPptal/consultas.php",
                                                                            data: form_data,
                                                                            success: function (response)
                                                                            {
                                                                                ...
                                                                                if (response == 1)
                                                                                {
                                                                                    ...
                                                                                    var form_data = {estruc: 24, tercero: tercero, id_comp_cnt: respuesta,
                                                                                        tipocomprobante: tipocomprobante, fecha: fecha, id_comprobante_pptal:
                                                                                                id_comprobante_pptal, numero: numero, devengos: devengos, aportes: aportes};
                                                                                    $.ajax({
                                                                                        type: "POST",
                                                                                        url: "estructura_aplicar_retenciones.php",
                                                                                        data: form_data,
                                ...
                                $(document).ready(function ()
                                {
                                    $('#btnEnviar').click(function ()
                                    {
                                        ...
                                        var idCompCnt = $("#respuesta").val();
                                            ...
                                            success: function (response)
                                            {
                                                //Verifica Si Comprobante Aplica Retencion
                                                console.log('Aplica retencion'+ response);
                                                if (response == 1)
                                                {
                                                    ...
                                                    var form_data = {estruc: 11, egreso: $("#id_comp_pptal_GE").val()};
                                                    $.ajax({
                                                        type: "POST",
                                                        url: "jsonPptal/consultas.php",
                                                        data: form_data,
                                                        success: function (response)
                                                        {
                                                            console.log('Tiene Interfaz'+ response);
                                                            if (response == 1)
                                                            {
                                                                $("#btnEnviar").attr("disabled", true);
                                                                var form_data = {action: 1, egr: $("#id_comp_pptal_GE").val()};
                                                                $.ajax({
                                                                    type: "POST",
                                                                    url: "jsonPptal/gf_egresoJson.php",
                                                                    data: form_data,
                            ...
                            <script type="text/javascript"> //Código JS para asignar un comprobante a partir de un tercero.
                                ...
                                $(document).ready(function ()
                                {
                                    ...
                                    {
                                        //Verificar Embargo 
                                        var form_data = {id_tercero: +$("#tercero").val(), action :15};
                                        $.ajax({
                                            type: "POST",
                                            url: "jsonPptal/gf_egresoJson.php",
                                            data: form_data,
                                            ...
                                            {
                                                if(response ==1){
                                                    $("#mensaje").html('Tercero Tiene Embargo');
                                                    $("#myModalError").modal("show");
                                                     $("#btnErrorModal").click(function(){
                                                         $("#myModalError").modal("hide");
                                                    })
                        ...
                        <script type="text/javascript">
                            ...
                            $(document).ready(function ()
                            {
                                $("#solicitudAprobada").change(function ()
                                {
                                    ...
                                    <?php if (!empty($_SESSION['nuevo_GE'])) { ?>
                                        $("#fecha").val("");
                                        form_data = {action: 2, id: $("#solicitudAprobada").val()}
                                        $.ajax({
                                            type: "POST",
                                            url: "jsonPptal/gf_egresoJson.php",
                                            data: form_data,
...
?>
                        ...
                        <div class="col-sm-12" style="margin-top:5px; padding: 0px;" > 
                            ...
                            <div class="valorT" style="font-size: 12px;" align="right">
                                <span class="col-md-10 col-sm-10" style="margin-left: 70px;text-align: right;"> Valor Total Aprobado:  </span>
                                ...
                                <?php }  else {
                                    echo '<input type="hidden" id="valorTotal" value="0"> ';
                                }?>   
                            </div>
                            <?php 
                            if($tipocomp==2){ 
                            if(!empty($_SESSION['nuevo_GE']) && !empty($_SESSION['id_comp_pptal_GE'])) {
                                #*** Valor Retención ***#
                                echo '<input type="hidden" id="comprobante_id" value="'.$_SESSION['id_comp_pptal_GE'].'"> ';
                                echo '<div align="right" class="form-group form-inline">';
                                echo '<span class="col-md-10 col-sm-10" style="margin-left: 70px;text-align: right;"> Valor Retención:  </span>';
                                echo '<div align="right" class="form-group form-inline">';
                                echo '<label style="margin-right: 10px;" id="valorR"></label>';
                                echo '<input type="hidden" id="valorRetencion" >';
                                echo '</div>';
                                echo '</div>';
                                
                                #*** Incluir Campo Para Poner El Valor A Girar ***#
                                #** Buscar Valor Abono **#
                                $va = $con->Listar("SELECT valor_abono FROM gf_comprobante_pptal WHERE id_unico = ".$_SESSION['id_comp_pptal_GE']);
                                if(empty($va[0][0])){
                                    $valorabono =0;
                                } else {
                                    $valorabono =$va[0][0];
                                }
                                echo '<div align="right" class="form-group form-inline" style="margin-top: -15px;">';
                                echo '<span class="col-md-10 col-sm-10" style="margin-left: 70px;text-align: right;"> Valor A Girar:  </span>';
                                echo '<div align="right" class="form-group form-inline">';
                                echo '<input type="text" id="valorG" onchange="cambiarV()" value="'.$valorabono.'"  class="form-control col-md-2 col-sm-2" style="width:80px; height:20px">';
                                echo '</div>';
                                echo '</div>';
                                
                                #*** Saldo Por Aplicar ***#
                                echo '<div align="right" class="form-group form-inline" style="margin-top: -15px;">';
                                echo '<span class="col-md-10 col-sm-10" style="margin-left: 70px;text-align: right;">Saldo Por Aplicar:  </span>';
                                echo '<div align="right" class="form-group form-inline">';
                                echo '<label style="margin-right: 10px;" id="saldoA"></label>';
                                echo '<input type="hidden" id="saldoAplicar">';
                                echo '</div>';
                                echo '</div>';
                                ?>
                                <script>
                                    $(document).ready(function (){
                                        var form_data = {action: 13, id: $("#comprobante_id").val() };
                                        $.ajax({
                                            type: "POST",
                                            url: "jsonPptal/gf_egresoJson.php",
                                            data: form_data,
                                            success: function (response)
                                            {
                                                var valor = formatV(response);
                                                $("#valorR").html(valor);
                                                $("#valorRetencion").val(response);
                                                var valorT = parseFloat($("#valorTotal").val());
                                                if($("#valorG").val()==""){
                                                    var valorG = parseFloat(0);
                                                } else {
                                                    var valorG = parseFloat($("#valorG").val());
                                                }
                                                if($("#valorRetencion").val()==""){   
                                                    var valorR =  parseFloat(0);
                                                } else {
                                                    var valorR = parseFloat($("#valorRetencion").val());
                                                }
                                                console.log(valorRetencion+'retencion');
                                                var SaldoA =  parseFloat(valorT-valorR-valorG);
                                                var valorA = formatV(SaldoA);
                                                $("#saldoA").html(valorA);
                                                $("#saldoAplicar").val(SaldoA);
                                            }
                                        });
                                    })
                                    function cambiarV(){
                                        var form_data = {action: 14, id: $("#comprobante_id").val(),valor:$("#valorG").val() };
                                        $.ajax({
                                            type: "POST",
                                            url: "jsonPptal/gf_egresoJson.php",
                                            data: form_data,
                                        ...
                                        var SaldoA = valorT-valorG-valorR;
                                        var valorA = formatV(SaldoA);
                                        $("#saldoA").html(valorA);
                                        $("#saldoAplicar").val(SaldoA);
