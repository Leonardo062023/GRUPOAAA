...
switch ($_SESSION['conexion']) {
    case '1':
      $row = $con->Listar("SELECT Id_Unico, Nombre, clase_sia FROM gf_clase_retencion ORDER BY Nombre ASC");
    break;
    case '2':
      $row = $con->Listar("SELECT Id_Unico, Nombre, clase_sia FROM gf_clase_retencion ORDER BY Nombre ASC");
    break;
          ...
          <table id="tabla" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
            ...
            <?php
               ...
               <tr>
                ...
                <td><a href="#" onclick="javascript:eliminarContrato(<?php echo $row[$i][0];?>);"><i title="Eliminar" class="glyphicon glyphicon-trash"></i></a>
                    <a href="EDITAR_GF_CLASE_RETENCION.php?ide=<?php echo ($row[$i][0]);?>"><i title="Modificar" class="glyphicon glyphicon-edit" ></i></a></td>
                <td><?php echo ucwords(mb_strtolower($row[$i][1]));?></td>
                <td><?php if($row[$i][2]==1) { echo 'Descuento Retenciones';} else {echo 'Otros Descuentos';}?></td>
      ...
      function eliminarContrato(id)
      {
        $("#myModal").modal('show');
                        $("#ver").click(function(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificarClaseRetencionJson.php?action==1",
                                data: form_data,
