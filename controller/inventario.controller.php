...
class inventarioController{
    ...
    public function obnterCodigoHijo(){
        ...
        list($codigo, $predecesor, $ncod, $codigo_hijo, $nivel) = array($data_padre[0][2], $data_padre[0][3], strlen($data_padre[0][2]), 0, 0);
        if(!empty($predecesor)){
            $data_gran_padre    = $this->plan->obnterPadreId($predecesor);
            $digitos_gran_padre = strlen($data_gran_padre[0][2]);
        }else{
    ...
    public function validarTipoInventario(){
        ...
        if(!empty($data[0][5])){
            ...
            $html .= "<option selected value=\"$data_tipo". [0][0] ." \">".ucwords(mb_strtolower($data_tipo[0][1]))."</option>";
    ...
    public function vistaConceptos(){
        ...
        $con = $this->tarifas->retornarConexion();
