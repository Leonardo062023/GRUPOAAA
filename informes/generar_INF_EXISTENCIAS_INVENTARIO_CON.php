    ...
    function NbLines($w,$txt){
     ...
     function Footer(){
        ...
        function Header(){
            ...
            $this->Ln(6);
            $this->Cell(200,5,utf8_decode('INFORME EXISTENCIAS DE INVENTARIO CONTEO'),0,0,'C');
            $this->Ln(5);
            ...
            $this->Cell(80,9,utf8_decode('ELEMENTO'),1,0,'C');
            $this->Cell(20,9,utf8_decode('UNIDAD'),1,0,'C');
            $this->Cell(22,9,utf8_decode('CANTIDAD'),1,0,'C');
            $this->Cell(25,9,utf8_decode('VALOR'),1,0,'C');
            $this->Cell(26,9,utf8_decode('CONTEO FISICO'),1,0,'C');
            $this->Ln(10);
        }
...
foreach ($dat as $row){
    ...
    $pdf->SetAligns(array('L', 'L', 'C', 'R', 'R','C'));
    $pdf->Setborder(array('1', '1', '1', '1', '1','1'));
    $pdf->SetWidths(array(25, 80, 20, 22, 25,26));
      $xsaldo = ROUND($xsaldo, 2);
        $xvalor = ROUND($xvalor, 2);
    if($xsaldo > 0 ){
      
        $pdf->fila(array($row[1], mb_strtoupper($row[2]), $row[3],utf8_decode($xsaldo),  number_format($xvalor, 2 , ',', '.'),('_____________')));
        $pdf->Ln(5);
        $xCant   += $xsaldo;
        $xValorT += $xvalor;
    }
...
$pdf->Cell(126,5,'Totales: ',0);
$pdf->Cell(27,5,$xCant,0,0,'C');
