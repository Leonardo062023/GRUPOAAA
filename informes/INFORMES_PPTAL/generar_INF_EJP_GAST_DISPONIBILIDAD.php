...
{ 
    ...
    $this->SetFont('Arial','',8);
    $this->Cell(340,5,utf8_decode('EJECUCION DEL PRESUPUESTO DE GASTOS - DISPONIBILIDADES'),0,0,'C');
    $this->SetFont('Arial','B',8);
    ...
    $this->Cell(27,8,utf8_decode('PRESUPUESTO'),0,0,'C');#
    $this->Cell(27,9,utf8_decode('DISPONIBILIDADES'),1,0,'C');################
...
while ($filactas = mysqli_fetch_array($conejc)) 
{
    ...
    if ($p1 == 0  && $p2 == 0  && $p3 == 0 && $p4==0 && $p5==0 && $p6==0 && $p7==0 && $p8==0 && $p9==0 && $p10==0 && $p11==0 && $p12==0 && $p13==0)
        ...
        if($filactas['codrub']!=''){
            ...
            $pdf->Cell(20,4,utf8_decode($filactas['codrub']),1,0,'L');
        }
        ...
        if($filactas['codfte']!=''){
            ...
            $pdf->Cell(15,4,utf8_decode($filactas['codfte']),1,0,'L');
        }
        
        
        $pdf->Cell(27,4,number_format($p1,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p2,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p3,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p4,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p5,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p6,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p7,2,'.',','),1,0,'R');
        $pdf->Cell(27,4,number_format($p8,2,'.',','),1,0,'R');
        $total1=$p8+$total1;
         $pdf->Ln($h);
        }
}
...
while (ob_get_length()) {
 ...
 ######### FIN FIRMAS #########
 $pdf->Output(0,utf8_decode('Informe_Ejecuciones_Pptales_Gastos_Disponibilidad('.date('d-m-Y').').pdf'),0);
?>