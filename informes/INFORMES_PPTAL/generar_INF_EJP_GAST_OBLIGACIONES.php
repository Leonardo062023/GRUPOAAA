...
{ 
    ...
    $this->SetFont('Arial','',8);
    $this->Cell(340,5,utf8_decode('EJECUCION DEL PRESUPUESTO DE GASTOS - OBLIGACIONES'),0,0,'C');
    $this->SetFont('Arial','B',8);
    ...
    $this->Cell(27,8,utf8_decode('PRESUPUESTO'),0,0,'C');#
    $this->Cell(27,9,utf8_decode('OBLIGACIONES'),1,0,'C');################
...
while ($filactas = mysqli_fetch_array($conejc)) 
{
    ...
    if ($p1 == 0  && $p2 == 0  && $p3 == 0 && $p4==0 && $p5==0 && $p6==0 && $p7==0 && $p8==0 && $p9==0 && $p10==0 && $p11==0 && $p12==0 && $p13==0)
        { } else {
        $pdf->CellFitScale(20,4,utf8_decode($filactas['codrub']),1,0,'R');
        $y = $pdf->GetY();
        $x = $pdf->GetX();        
        ...
        $pdf->CellFitScale(27,4,number_format($p1,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($p2,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($p3,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($p4,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($p5,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($p6,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($p11,2,'.',','),1,0,'R');
        $pdf->CellFitScale(27,4,number_format($t,2,'.',','),1,0,'R');
        $total1=$t+$total1;
         $pdf->Ln($h);
        }
}
...
while (ob_get_length()) {
 ...
 ######### FIN FIRMAS #########
 $pdf->Output(0,utf8_decode('Informe_Ejecuciones_Pptales_Gastos_Obligaciones('.date('d-m-Y').').pdf'),0);
?>