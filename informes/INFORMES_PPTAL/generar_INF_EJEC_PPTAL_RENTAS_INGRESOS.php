...
{ 
    ...
    $this->Cell(80,9,utf8_decode('DETALLE'),1,0,'C');#
    $this->Cell(24,9,utf8_decode('FUENTE'),1,0,'C');#
    $this->Cell(35,4,utf8_decode('PAC'),0,0,'C');#
    $this->Cell(72,4,utf8_decode('MODIFICACIONES PAC'),0,0,'C');#
    $this->Cell(45,4,utf8_decode('PAC'),0,0,'C');#
    $this->Cell(45,4,utf8_decode('PAC'),0,0,'C');#
    ...
    $this->Cell(80,5,utf8_decode(''),0,0,'C');#
    ...
    $this->Cell(45,5,utf8_decode('DEFINITIVO'),0,0,'C');
    $this->Cell(45,5,utf8_decode('RECAUDADO'),0,0,'C');#
...
if(empty($_REQUEST['niveles'])) { 
    ...
    $sql2 = "SELECT DISTINCT 
                        ...
                        cod_fuente          as fuente 
    from temporal_consulta_pptal_gastos 
    WHERE length(REPLACE(cod_rubro, '.', '')) <= '".$_REQUEST['niveles']."' 
...
while ($filactas = mysqli_fetch_array($conejc)) 
{
    ...
    if ($p1 == 0  && $p2 == 0  && $p3 == 0 && $p4==0 && $p5==0 && $p6==0)
        ...
        if($filactas['codrub']!="")
            $pdf->cellfitscale(28,4,utf8_decode($filactas['codrub']),0,0,'L');
        else
            $pdf->Cell(28,4,'',0,0,'L');
        $y = $pdf->GetY();
        $x = $pdf->GetX();        
        $pdf->MultiCell(80,4,utf8_decode($filactas['nomrub']),0,'L');
        $y2 = $pdf->GetY();
        $h = $y2-$y;
        $px = $x + 80;
        $pdf->Ln(-$h);
        ...
        $pdf->cellfitscale(45,4,number_format($p4,2,'.',','),0,0,'R');
        $pdf->cellfitscale(45,4,number_format($p5,2,'.',','),0,0,'R');
        $pdf->Ln($h);
        #Actualización 10/02/2017
}
}
