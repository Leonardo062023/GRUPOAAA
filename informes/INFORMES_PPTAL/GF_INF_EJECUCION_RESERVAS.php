...
# ***************************************** Modificaciones ***************************************** #
######################################################################################################
#09/02/2018 | Erica G. | Archivo Creado
######################################################################################################
...
$ctas = "SELECT DISTINCT
         ...
         WHERE rpp.codi_presupuesto BETWEEN '$codigoI' AND '$codigoF' 
             AND ((rpp.tipoclase = 16) OR (rpp.tipoclase=7 AND rpp.tipovigencia = 3)) 
...
{ 
    ...
    $this->Ln(5);
    $this->SetFont('Arial','',8);
    $this->Cell(340,5,utf8_decode('EJECUCION PRESUPUESTAL DE RESERVAS'),0,0,'C');
    $this->SetFont('Arial','B',8);
    ...
    $this->CellFitScale(80,9,utf8_decode('DETALLE'),1,0,'C');#
    $this->CellFitScale(45,9,utf8_decode('FUENTE'),1,0,'C');################
    ...
    $this->CellFitScale(80,5,utf8_decode(' '),0,0,'C');#
    $this->CellFitScale(45,5,utf8_decode(' '),0,0,'C');###############################
    ...
    $this->CellFitScale(20,5,utf8_decode('OBLIGACIONES'),0,0,'C');#
...
$pdf->setX(20);    
    ...
    if ($p1 == 0  && $p2 == 0  && $p3 == 0 && $p4==0 && $p5==0 && $p6==0 && $p11==0 && $p12==0)
        ...
        $y = $pdf->GetY();
        $x = $pdf->GetX();        
        $pdf->MultiCell(80,5,utf8_decode($filactas['nomrub']),0,'L');        
        $y2 = $pdf->GetY();
        $h1 = $y2-$y;
        $pdf->Ln(-$h1);
        $pdf->SetX($x+80);
        $pdf->MultiCell(45,5,utf8_decode($filactas['codfte']),0,'L');        
        $y3 = $pdf->GetY();
        $h2 = $y3-$y;
        $h = max($h1, $h2);
        $px = $x + 125;
        ...
        if(empty($p2)) {
         ...
         if(empty($p11)) {
            $pdf->Cell(20,$h,number_format($p11,2,'.',','),1,0,'R');
        } else {
            $pdf->CellFitScale(20,$h,number_format($p11,2,'.',','),1,0,'R');
        }
...
$x = $pdf->GetX();
 ...
 ######### FIN FIRMAS #########
 $pdf->Output(0,utf8_decode('Informe_Ejecucion_Reservas('.date('d-m-Y').').pdf'),0);
} elseif($_GET['tipo']==2){
 header("Content-type: application/vnd.ms-excel");
 header("Content-Disposition: attachment; filename=Informe_Ejecucion_Reservas.xls");?>

<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<title>Ejecución Reservas</title>
</head>
<body>
<table width="100%" border="1" cellspacing="0" cellpadding="0">
    <tr>
        <th colspan="11" align="center"><strong>
           ...
           <br/>&nbsp;
           <br/>EJECUCIÓN PRESUPUESTAL DE RESERVAS
    ...
    <tr>
        ...
        <td align="center" rowspan="2"><strong>TOTAL <br/>OBLIGACIONES</strong></td>
