...
(SELECT round(SUM(df.valor_total_ajustado), 4) FROM gp_detalle_factura df WHERE gp_factura.id_unico = df.factura) as total, 
...
//valores detallefactura
  ...
  $sqlDtf = " SELECT gp_concepto.nombre,gp_detalle_factura.valor_origen,
        ...
        gp_detalle_factura.valor_total_ajustado,gp_detalle_factura.valor,gp_detalle_factura.ajuste_peso, gp_detalle_factura.tipo_descuento , 
        gp_detalle_factura.descuento 
...
while ($rowDtf = mysqli_fetch_row($resultDtf)) {
    ...
    if ($rowDtf[8] > 0) {
        if($rowDtf[12]==1){
            ...
            #* Valor Base Real
            if($pb!=0){
                $vds = ($rowDtf[1]*$rowDtf[2])/(1+($pb/100));
            } else {
                $vds = ($rowDtf[1]*$rowDtf[2]);
            }
            $Descuento =($rowDtf[8]*100)/$vds;
        }
    }
    //items
    $Detalle_Factura[] = [
...
function factura($datosCodificados, $token, $mysqli, $id_factura) {
    ...
    curl_setopt_array($curl, [
        CURLOPT_PORT => "8443",
        CURLOPT_URL => "https://csi.clarisa.co:8443/facturacion/rest/api/v1/factura",
...
<script type="text/javascript">
    ...
    $("#ver1").click(function () {
        $("#myModal1").modal('hide');
        window.location = '../facturacionElectronica.php';
    });
</script>
