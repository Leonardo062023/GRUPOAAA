...
{ 
  ...
  switch ($_SESSION['conexion']) {
    case '1':
      $row = $con->Listar("SELECT Id_Unico, Nombre, Valor FROM gf_tipo_activo WHERE (Id_Unico) = '$id_tipo_activo'");
    break;
    case '2':
      $row = $con->Listar("SELECT Id_Unico, Nombre, Valor FROM gf_tipo_activo WHERE (Id_Unico) = '$id_tipo_activo'");
  break;
          ...
          <p align="center" style="margin-bottom: 25px; margin-top: 25px; margin-left: 30px; font-size: 80%">Los campos marcados con <strong style="color:#03C1FB;">*</strong> son obligatorios.</p>
            ...
            <div class="form-group" style="margin-top: -10px;">
              <label for="valor" class="col-sm-5 control-label"><strong style="color:#03C1FB;">*</strong>Valor:</label>
                <input type="text" name="valor" id="valor" class="form-control" maxlength="19" title="Ingrese el valor del activo" onkeypress="return txtValida(event, 'dec', 'valor', '2')" placeholder="Valor" value="<?php echo $row[$i][2]?>" required>
            </div>
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificar_GF_TIPO_ACTIVOJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
