...
$ficha = $con->Listar("SELECT id_unico,descripcion from gf_ficha where id_unico=$ficha");

$idFicha = $ficha[0][0];
$descripcion = $ficha[0][1];
...
<body onload="limpiarCampos()">
    <div class="container-fluid text-left">
        <div class="row content">
            ...
            <div class="col-sm-10 text-center" style="margin-top:-22px;">
                ...
                <a href="<?php echo "modificar_GF_FICHA.php?action=1&id=" . base64_encode($idFicha); ?>" class="glyphicon glyphicon-circle-arrow-left" style="display:inline-block;margin-left:10px; font-size:150%; vertical-align:middle;text-decoration:none" title="Volver"></a>
                ...
                <div class="col-sm-8 col-sm-offset-3 text-left" style="margin-bottom:-16px">
                    <div class="client-form">
                        <form name="form" id="form" class="form-horizontal" method="POST" enctype="multipart/form-data" style="margin-top:-15px" action="javascript:agregar()">
                            ...
                            <div class="col-sm-2" style="margin-right:70px">
                                ...
                                <select class="form-control input-sm" name="sltElementoFicha" id="sltElementoFicha" title="Seleccione elemento ficha" style="width:150px;height:26px;padding:2px;cursor: pointer" required>
                                    ...
                                    <?php
                                    $result = $con->Listar("SELECT id_unico,nombre FROM gf_elemento_ficha");
                                    for ($fila = 0; $fila < count($result); $fila++) { ?>
                                        <option value="<?php echo $result[$fila][0] ?>"><?php echo ucwords(strtolower($result[$fila][1])) ?></option>
                                    <?php }
                                    ?>
                                </select>
                            </div>
                            ...
                            <div class="col-sm-1" style="margin-top:30px">
                                <button type="submit" id="btnGuardar" class="btn sombra btn-primary" title="Guardar cierre contable">
                ...
                <div class="col-sm-12 table-responsive contTabla" style="margin-top:10px">
                    <table id="tabla" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            <?php $result = $con->Listar("SELECT    fi.id_unico,
                                                        fi.ficha,
                                                        fi.obligatorio,
                                                        fi.autogenerado,
                                                        elm.id_unico,
                                        ...
                                        WHERE f.id_unico = '" . $_REQUEST['ficha'] . "'");
                            for ($row = 0; $row < count($result); $row++) {  ?>
                                <tr>
                                    <td class="oculto">
                                        <?php echo $result[$row][0]; ?>
                                    </td>
                                    <td class="campos ">
                                        <a href="#<?php echo $result[$row][0]; ?>" onclick="javascript:eliminar(<?php echo $result[$row][0]; ?>)" title="Eliminar">
                                            <li class="glyphicon glyphicon-trash"></li>
                                        </a>
                                        <a href="#<?php echo $result[$row][0]; ?>" title="Modificar" id="mod" onclick="javascript:modificar(<?php echo $result[$row][0]; ?>);">
                                            <li class="glyphicon glyphicon-edit"></li>
                                        </a>
                                    </td>
                                    <td class="campos text-left">
                                        <?php
                                        echo '<label class="valorLabel col-sm-12" style="font-weight:normal"  id="lblElementoFicha' . $result[$row][0] . '">' . ucwords(strtolower($result[$row][5])) . '</label>';
                                        ?>
                                        <select style="display: none;padding:2px" class="col-sm-10 campoD" id="sltelementoficha<?php echo $result[$row][0]; ?>">
                                            <option value="<?php echo $result[$row][4]; ?>"><?php echo ucwords(strtolower($result[$row][5])); ?></option>
                                            <?php
                                            $resultE = $con->Listar("select elm.id_unico,elm.nombre from gf_elemento_ficha elm where elm.id_unico!= " . $result[$row][4] . " ");
                                            for ($rowE = 0; $rowE < count($resultE); $rowE++) { ?>
                                                <option value="<?php echo $resultE[$rowE][0] ?>"> <?php echo ucwords(strtolower($resultE[$rowE][1])) ?></option>
                                            <?php }
                                            ?>
                                        </select>
                                    </td>
                                    <td class="campos text-left">
                                        <?php
                                        $obligatorio = (int) $result[$row][2];
                                        switch ($obligatorio) {
                                            case 1:
                                                echo '<label class="valorLabel col-sm-12" style="font-weight:normal"  id="lblObligatorio' . $result[$row][0] . '">SI</label>'; ?>
                                                <script type="text/javascript">
                                                    $(document).ready(function() {
                                                        $("input:radio[name=optObligatorio<?php echo $result[$row][0]; ?>]").filter("[value=1]").prop("checked", true);
                                                    });
                                                </script>
                                                <div class="col-sm-1 radios" id="radios<?php echo $result[$row][0]; ?>" style="display: none">
                                                    <input type="radio" name="optObligatorio<?php echo $result[$row][0]; ?>" value="1" id="optOblA<?php echo $result[$row][0]; ?>" title="Indique si es obligatorio" checked>SI
                                                    <input type="radio" name="optObligatorio<?php echo $result[$row][0]; ?>" value="2" id="optOblB<?php echo $result[$row][0]; ?>" title="Indique si no es obligatorio">NO
                                                </div>
                                            <?php
                                                break;
                                            case 2:
                                                echo '<label class="valorLabel col-sm-12" style="font-weight:normal"  id="lblObligatorio' . $result[$row][0] . '">NO</label>'; ?>
                                                <script type="text/javascript">
                                                    $(document).ready(function() {
                                                        $("input:radio[name=optObligatorio<?php echo $result[$row][0]; ?>]").filter("[value=2]").prop("checked", true);
                                                    });
                                                </script>
                                                <div class="col-sm-1 " style="display: none" id="radios<?php echo $result[$row][0]; ?>">
                                                    <input type="radio" name="optObligatorio<?php echo $result[$row][0]; ?>" value="1" id="optOblA<?php echo $result[$row][0]; ?>" title="Indique si es obligatorio" />SI
                                                    <input type="radio" name="optObligatorio<?php echo $result[$row][0]; ?>" value="2" id="optOblB<?php echo $result[$row][0]; ?>" title="Indique si no es obligatorio" checked />NO
                                                </div>
                                        <?php
                                                break;
                                        }
                                    ...
                                    <td class="campos text-left">
                                        ...
                                        switch ($result[$row][3]) {
                                            case 1:
                                                echo '<label class="valorLabel col-sm-12" style="font-weight:normal"  id="lblAutoG' . $result[$row][0] . '">SI</label>'; ?>

                                                <script type="text/javascript">
                                                    $(document).ready(function() {

                                                        $("input:radio[name=optAutoGenerado<?php echo $result[$row][0]; ?>]").filter("[value=1]").prop("checked", true);

                                                    });
                                                </script>
                                                <div class="col-sm-10 radios" id="radiosGenerado<?php echo $result[$row][0]; ?>" style="display: none">
                                                    <input type="radio" name="optAutoGenerado<?php echo $result[$row][0]; ?>" value="1" id="optAutoGA<?php echo $result[$row][0]; ?>" title="Indique si es obligatorio" checked />SI
                                                    <input type="radio" name="optAutoGenerado<?php echo $result[$row][0]; ?>" value="2" id="optAutoGB<?php echo $result[$row][0]; ?>" title="Indique si no es obligatorio" />NO
                                                </div>
                                            <?php
                                                break;
                                            case 2:
                                                echo '<label class="valorLabel col-sm-12" style="font-weight:normal"  id="lblAutoG' . $result[$row][0] . '">NO</label>'; ?>
                                                <script type="text/javascript">
                                                    $(document).ready(function() {
                                                        $("input:radio[name=optAutoGenerado<?php echo $result[$row][0]; ?>]").filter("[value=2]").prop("checked", true);
                                                    });
                                                </script>
                                                <div class="col-sm-10 radios" id="radiosGenerado<?php echo $result[$row][0]; ?>" style="display: none">
                                                    <input type="radio" name="optAutoGenerado<?php echo $result[$row][0]; ?>" value="1" id="optAutoGA<?php echo $result[$row][0]; ?>" title="Indique si es obligatorio" />SI
                                                    <input type="radio" name="optAutoGenerado<?php echo $result[$row][0]; ?>" value="2" id="optAutoGB<?php echo $result[$row][0]; ?>" title="Indique si no es obligatorio" checked />NO
                                                </div>
                                        <?php
                                                break;
                                            default:
                                        ...
                                        ?>
                                        <div>
                                            <table id="tab<?php echo $result[$row][0] ?>" class="col-sm-1" style="padding:0px;background-color:transparent;background:transparent;">
                                                <tbody>
                                                    <tr style="background-color:transparent;">
                                                        <td style="background-color:transparent;">
                                                            <a href="#<?php echo $result[$row][0]; ?>" title="Guardar" id="guardar<?php echo $result[$row][0]; ?>" style="display: none;" onclick="javascript:guardarCambios(<?php echo $result[$row][0]; ?>)">
                                                                <li class="glyphicon glyphicon-floppy-disk"></li>
                                                            </a>
                                                        </td>
                                                        <td style="background-color:transparent;">
                                                            <a href="#<?php echo $result[$row][0]; ?>" title="Cancelar" id="cancelar<?php echo $result[$row][0] ?>" style="display: none" onclick="javascript:cancelar(<?php echo $result[$row][0]; ?>)">
    ...
    <script type="text/javascript">
        ...
        function eliminar(id) {
            ...
            $("#ver").click(function() {
                ...
                $.ajax({
                    type: "GET",
                    url: "json/modificarFichaInventarioJson?action=1&id=" + id,
        ...
        function guardarCambios(id) {
            ...
            $.ajax({
                type: 'POST',
                url: "json/modificarFichaInventarioJson?action=3.php",
                data: form_data,
                success: function(data) {
                    result = JSON.parse(data);
                    console.log(data);
    ...
    <script type="text/javascript">
        function agregar() {
            ...
            $.ajax({
                type: 'POST',
                url: "json/modificarFichaInventarioJson.php?action=2",
                data: formData,
                contentType: false,
                processData: false,
