...
if($mysqli->real_escape_string(''.$_POST['txtnumerohoras'].'')=="")
    $numerohoras = "null";
else
    $numerohoras    = '"'.$mysqli->real_escape_string(''.$_POST['txtnumerohoras'].'').'"';
...
$sql = "INSERT INTO gn_horas_extras(numerohoras,fecha,empleado,concepto) VALUES ($numerohoras,$fecha,$empleado,$concepto)";
$resultado = $mysqli->query($sql);
if ($resultado==true) {
  $sqlId="SELECT MAX(id_unico) FROM gn_horas_extras";
  $ultmId = $mysqli->query($sqlId);
  $rowId=mysqli_fetch_row($ultmId);
  $id_horaE=$rowId[0];
  $agr = agregarHoraE($id_horaE);
}
if(empty($_POST['txtId']))
{
    $las = "SELECT MAX(id_unico) FROM gn_horas_extras";
...
<script type="text/javascript">
  ...
  $("#ver1").click(function(){
    $("#myModal1").modal('hide');      
        window.location='../registrar_GN_HORAS_EXTRAS.php?idE=<?php echo md5($_POST['sltEmpleado'])?>';
      //window.history.go(-1);
  });
</script>
<?php }else{ ?>
