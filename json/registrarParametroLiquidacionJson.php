...
$diaspv    = empty($diaspv) ? 0 : $diaspv;
 ...
 hora_extra_no, tope_aux_transporte , dias_primav ) 
  ...
  $provision,$tipo_e,$hextnor,$tauxt, $diaspv)";
$resultado = $mysqli->query($sql);
if ($resultado==true) {
  $sqlId="SELECT MAX(id_unico) FROM gn_parametros_liquidacion";
  $ultmId = $mysqli->query($sqlId);
  $rowId=mysqli_fetch_row($ultmId);
  $id_para=$rowId[0];
  $agr = agregarParametro($id_para);
}
