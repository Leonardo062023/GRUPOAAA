...
#   ****************************    Modificaciones      ****************************    #
##########################################################################################
#15/08/2018 |Erica G. |Usuario
#05/06/2018 |Erica G. |Buscar Configuración Concepto Tabla Verficar Código 
#11/07/2017 |ERICA G. |REGISTRAR RETENCIONES
# Modificado por : Alexander Numpaque       Fecha de Modificación : 03/05/2017
# Descripción    : Se agrego validación para evitar que el usuario ingrese un valor mayor al valor registrado en el concepto
##################################################################################################################################################################
        ...
        <div class="row content">
            ...
            <div class="col-sm-8 col-xs-12 text-left">
                ...
                <div style="margin-top:-7px; border:4px solid #020324;border-radius: 10px;margin-left: 4px;margin-right: 4px;" class="client-form">
                    ...
                    <form name="form" class="form-horizontal" method="POST" style="margin-left:30px"  enctype="multipart/form-data" action="json/registrarPagoFacturaJson.php">
                        ...
                        <div class="form-inline form-group" style="margin-top:5px">
                            ...
                            <label for="txtNumeroR" class="control-label col-sm-2">
                                <strong class="obligado">*</strong>Nro Recuado:
                            </label>
                        ...
                        <div class="form-group form-inline" style="margin-top:-15px">
                            ...
                            <select name="sltBanco" id="sltBanco" class="col-sm-2 form-control input-sm" style="width:100px;cursor:pointer;height:30px" title="Seleccione banco" required>
                                ...
                                if(!empty($banco)){
                                    ...
                                    cargar_combos($sql2);
                                    $sql3 = "SELECT  ctb.id_unico,CONCAT(ctb.numerocuenta,' ',ctb.descripcion)
                                            FROM gf_cuenta_bancaria ctb
                                            LEFT JOIN gf_cuenta_bancaria_tercero ctbt ON ctb.id_unico = ctbt.cuentabancaria
                                            WHERE ctbt.tercero ='". $_SESSION['compania']."' AND ctb.id_unico!=$banco AND ctb.parametrizacionanno = $anno ORDER BY ctb.numerocuenta";
                                    cargar_combos($sql3);
                        ...
                        <div class="form-group form-inline" style="margin-top:-15px">
                            ...
                            <select name="sltTercero" id="sltTercero" class="form-control col-sm-2 input-sm" title="Seleccione un responsable" style="width:342px;cursor:pointer;height:30px" required="required">
                                <?php
                                if(!empty($tercero)){
                                    $sql18 = "SELECT  IF(t.razonsocial IS NULL OR t.razonsocial ='', 
                                    ...
                                    echo '<option value="'.$row18[1].'">'.ucwords(mb_strtolower($row18[0])).'</option>';
                                    $sql19 = "SELECT t.id_unico, IF(t.razonsocial IS NULL OR t.razonsocial ='', 
                                        CONCAT_WS(' ', t.nombreuno, t.nombredos, t.apellidouno, t.apellidodos, ' - ',t.numeroidentificacion,'  ', t.digitoverficacion),
                                        CONCAT_WS(' - ',t.razonsocial, t.numeroidentificacion, t.digitoverficacion)) 
                                        FROM gf_tercero t 
                                        WHERE t.compania = $compania  ORDER BY numeroidentificacion = '9999999999' DESC LIMIT 20";
                                    $rs19 = $mysqli->query($sql19);
                                    while($row19 = mysqli_fetch_row($rs19)){
                                        echo '<option value="'.$row19[0].'">'.ucwords(mb_strtolower($row19[1])).'</option>';
                                    }
                                }else{
                                    echo '<option value="">Tercero</option>';
                                    $sql1 = "SELECT t.id_unico, IF(t.razonsocial IS NULL OR t.razonsocial ='', 
                                        CONCAT_WS(' ', t.nombreuno, t.nombredos, t.apellidouno, t.apellidodos, ' - ',t.numeroidentificacion,'  ', t.digitoverficacion),
                                        CONCAT_WS(' - ',t.razonsocial, t.numeroidentificacion, t.digitoverficacion)) 
                                        FROM gf_tercero t 
                                        WHERE t.compania = $compania ORDER BY numeroidentificacion = '9999999999' DESC LIMIT 20";
                        ...
                        <div class="form-group form-inline" style="margin-top:-15px;margin-bottom:5px;">
                            ...
                            <select name="sltBuscarT" id="sltBuscarT" onchange="buscarRecaudos()" class="form-control col-sm-1" style="width: 100px" title="Buscar recaudo">
                                ...
                                echo "<option value>Tipo Recuado</option>";
                                ...
                                <select name="sltFactura2" id="sltFactura2" class="form-control input-sm" style="width:170px;height:30px;" title="Seleccione  factura" required="" onclick="load_value_input('',$('#sltFactura2').val())">
                                    ...
                                    #Inclusión nueva validamos que si existe un tercero que de ese tercero me traiga las facturas relacionadas que tengan saldo
                                        if(!empty($tercero)){
                                            ...
                                            $sqlF = "SELECT f.id_unico, tf.prefijo,  f.numero_factura,
                                                (SELECT SUM(df.valor_total_ajustado) from gp_detalle_factura df  WHERE df.factura = f.id_unico) - 
                                                (SELECT IF(SUM(dp.valor) is null, 0, SUM(dp.valor))+IF(SUM(dp.iva) is null, 0, SUM(dp.iva))+IF(SUM(dp.impoconsumo) is null, 0, SUM(dp.impoconsumo)) from gp_detalle_pago dp left join gp_detalle_factura df ON dp.detalle_factura = df.id_unico  where df.factura = f.id_unico) dp ,date_format(f.fecha_factura, '%d/%m/%Y'), uv.codigo_interno
                                                FROM  gp_factura f 
                                                LEFT JOIN gp_tipo_factura tf ON f.tipofactura = tf.id_unico 
                                                LEFT JOIN gp_unidad_vivienda_medidor_servicio uvms ON uvms.id_unico = f.unidad_vivienda_servicio
                                                ...
                                                where f.tercero= $tercero AND f.fecha_factura <='".$fecha."'
                                                HAVING (SELECT SUM(df.valor_total_ajustado) from gp_detalle_factura df  WHERE df.factura = f.id_unico) - 
                                                (SELECT IF(SUM(dp.valor) is null, 0, SUM(dp.valor))+IF(SUM(dp.iva) is null, 0, SUM(dp.iva))+IF(SUM(dp.impoconsumo) is null, 0, SUM(dp.impoconsumo)) from gp_detalle_pago dp left join gp_detalle_factura df ON dp.detalle_factura = df.id_unico  where df.factura = f.id_unico) != 0";
                                            $resultF = $mysqli->query($sqlF);
                                            $val = array();
                                            $v =0;
                                            while($fila = mysqli_fetch_row($resultF)){
                                                echo '<option value="'.$fila[0].'">'.ucwords(mb_strtoupper($fila[1].' '.$fila[2].' Saldo: '.number_format($fila[3],2, ',', '.').' '.$fila[4].' - Código I: '.$fila[5])).'</option>';
                                            }
                                        }else{
//                                            #Consulta que trae las facturas pero distingue los valores de acuerdo asus afectaciones
            ...
            <div class="col-sm-8" style="margin-top:5px;">
                ...
                <div class="col-sm-1 col-sm-offset-1" style="margin-right:30px">
                    <!-- Inicio de label -->
        ...
        $.ajax({
            ...
            success: function(data,textStatus,jqXHR) {
                ...
                if(data == 1){
                    ...
                    $("#btnEditar, #btnPay, #btnVerRet, #mod, #del, #btnDetalle,#btnRet").attr('disabled',true);
                    $("#btnEditar").removeAttr('onclick');
                    $("#btnPay").removeAttr('onclick');
                    $("#btnRet").removeAttr('onclick');
                    $("#btnVerRet").removeAttr('onclick');
                    ...
                    $("#del").removeAttr('onclick');
                    $("#btnDetalle").removeAttr('onclick');
                    $("#btnEditar, #btnPay, #mod, #del, #btnDetalle,#btnRet,#btnVerRet").removeAttr('title');
        ...
        $.ajax({
            ...
            success: function(data,textStatus,jqXHR) {
                if(data == true){
                    ...
                    $("#btnEditar, #btnPay, #mod, #del, #btnDetalle, #btnEliminar,#btnRet, #btnVerRet").attr('disabled',true);
                    $("#btnEditar, #btnPay, #mod, #del, #btnDetalle, #btnEliminar, #btnRet,#btnVerRet").removeAttr('onclick');
                    $("#btnEditar, #btnPay, #mod, #del, #btnDetalle, #btnEliminar, #btnRet,#btnVerRet").removeAttr('title');
                    $("#mod, #del").removeAttr('href');
                    $("#mod, #del").removeClass('active');
                }
            }
