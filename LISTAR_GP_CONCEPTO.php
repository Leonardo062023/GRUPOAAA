...
switch ($conexion) {
  case 1:
    $resultado = $con->Listar("SELECT c.id_unico, c.nombre, 
    tc.nombre, 
    top.nombre, 
  ...
  case 2:
    ...
    top.nombre,
    pi.codi, pi.nombre, fb.nombre,
    CASE WHEN c.alojamiento = 1 THEN 'SI' ELSE 'NO' END,
    ca.nombre,
    c.ajuste,
    c.traduccion
    FROM gp_concepto c
      ...
      <div class="col-sm-10 text-left">
        ...
        <div class="table-responsive" style="margin-left: 5px; margin-right: 5px;margin-top: -15px">
          <div class="table-responsive" style="margin-left: 5px; margin-right: 5px;">
            <table id="tabla" class="table table-striped table-condensed" class="display" cellspacing="0" width="100%">
              <thead>
                <tr>
                  ...
                  <?php if ($tr != 0) { ?>
                    <td class="cabeza"><strong>Ajuste</strong></td>
                    <td class="cabeza"><strong>Traducción</strong></td>
                  <?php } ?>
                </tr>
                <tr>
                  ...
                  <?php if ($tr != 0) { ?>
                    <th class="cabeza"><strong>Ajuste</strong></th>
                    <th class="cabeza"><strong>Traducción</strong></th>
                  <?php } ?>
                </tr>
              </thead>
              <tbody>
                ...
                for ($row = 0; $row < count($resultado); $row++) { ?>
                  <tr>
                    ...
                    <td class="campos">
                      ...
                      <a class="campos" href="Modificar_GP_CONCEPTO.php?action=1&id=<?php echo base64_encode($resultado[$row][0]); ?>"><i title="Modificar" class="glyphicon glyphicon-edit"></i></a>
                      <a class="campos" href="GP_CONCEPTO_TARIFA.php?id=<?php echo base64_encode($resultado[$row][0]); ?>"><i title="Tarifa" class="glyphicon glyphicon-usd"></i></a>
                    </td>
                    ...
                    <td class="campos"><?php echo ucwords(mb_strtolower($resultado[$row][3])); ?></td>
                    <td class="campos"><?php echo $resultado[$row][4] . ' - ' . ucwords(mb_strtolower(($resultado[$row][5]))); ?></td>
                    <td class="campos"><?php echo ucwords(mb_strtolower($resultado[$row][6])); ?></td>
                    <td class="campos"><?php echo $resultado[$row][7]; ?></td>
            ...
            <div align="right">
              <a href="Modificar_GP_CONCEPTO.php?action=2" class="btn btn-primary" style="box-shadow: 0px 2px 5px 1px gray;color: #fff; border-color: #1075C1; margin-top: 20px;margin-bottom: 20px; margin-left:-20px; margin-right:4px">Registrar Nuevo
              </a>
            </div>
          </div>
        </div>
  ...
  <script type="text/javascript">
    function eliminar(id) {
      ...
      $("#ver").click(function() {
        ...
        $.ajax({
          type: "POST",
          url: "json/modificar_GP_CONCEPTOJson.php",
          data: form_data,
