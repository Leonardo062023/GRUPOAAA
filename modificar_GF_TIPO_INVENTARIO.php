...
$id_tipo_inve = " ";
if (isset($_GET["id_tipo_inve"]))
{ 
  $id_tipo_inve = (($_GET["id_tipo_inve"]));

  
switch ($_SESSION['conexion']) {
  case '1':
    $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_tipo_inventario WHERE (Id_Unico) = '$id_tipo_inve'");
  break;
  case '2':
    $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_tipo_inventario WHERE (Id_Unico) = '$id_tipo_inve'");
break;
...
<div class="container-fluid text-center">
  ...
  <?php require_once 'menu.php'; ?>
    ...
    <div class="col-sm-10 text-left">
      ...
      <h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;">Modificar Tipo Inventario</h2>
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificar_GF_TIPO_INVENTARIOJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
