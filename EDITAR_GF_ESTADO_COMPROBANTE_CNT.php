...
$con = new ConexionPDO();
	...
	if(isset($_GET["id"])){
		...
		switch ($_SESSION['conexion']) {
			case '1':
			  $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_estado_comprobante_cnt WHERE (Id_Unico) = '$id'");
			break;
			case '2':
			  $row = $con->Listar("SELECT Id_Unico, Nombre FROM gf_estado_comprobante_cnt WHERE (Id_Unico) = '$id'");
		  break;
	...
	require_once 'head.php';
?>
	<title>Modificar Estado Comprobante Contable</title>
</head>
<body>
	<div class="container-fluid text-center">
		<div class="row content">
			...
			<div class="col-sm-10 text-left">
				...
				<!-- Inicio de Titulo -->
				<h2 id="forma-titulo3" align="center" style="margin-bottom: 20px; margin-right: 4px; margin-left: 4px;"">Modificar Estado Comprobante Contable</h2>
				<!-- Fin de Titulo -->
...
<script>
                        function modificar(){
                            ...
                            $.ajax({
                                type: 'POST',
                                url: "Json/modificarEstadoComprobanteCntJson.php?action=3",
                                data:formData,
                                contentType: false,
                                processData: false,
